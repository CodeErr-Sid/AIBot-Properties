(function ($) {
   "use strict"; gsap.registerPlugin(DrawSVGPlugin, ScrollTrigger, CSSRulePlugin, ScrollToPlugin, MorphSVGPlugin, CustomEase, InertiaPlugin, ScrollSmoother, TextPlugin, Flip); var mobileQuery = window.matchMedia('(max-width: 450px)'); var keys = { 37: 1, 38: 1, 39: 1, 40: 1 }; function preventDefault(e) { e.preventDefault(); }
   ...
   var siteHeader = $('.site-header'), page = $('main'), body = $('body'), mouseCursor = $('#mouseCursor'), cursorDark, cursorLight, cursorY, scrollSmoother, $cursorInner = ' <div class="mouse-cursor-icon"></div><svg height="100%" width="100%" viewbox="0 0 100 100"><circle class="main-circle" cx="50" cy="50" r="40" /></svg><span class="mouse-cursor-text"></span>'; mouseCursor.append($cursorInner); var matchMedia = gsap.matchMedia(), isPhone = '(max-width: 450px)', isTablet = '(min-width: 450px) and (max-width: 900px)', isDesktop = '(min-width: 900px)'; function naylaMouseCursor(init) {
      var mouseCursor = $('#mouseCursor'), $cursorInner = ' <div class="mouse-cursor-icon"></div><svg height="100%" width="100%" viewbox="0 0 100 100"><circle class="main-circle" cx="50" cy="50" r="40" /></svg><span class="mouse-cursor-text"></span>'; if (mouseCursor.length) {
         mouseCursor.append($cursorInner); let cursorText = mouseCursor.children('.mouse-cursor-text').wrapInner('<span></span>'), cursorIcon = mouseCursor.children('.mouse-cursor-icon'), yperc, cursorActive; gsap.set(mouseCursor, { xPercent: -50, yPercent: -50 }); let xTo = gsap.quickTo(mouseCursor, "x", { duration: 0.6, ease: "power3" }), yTo = gsap.quickTo(mouseCursor, "y", { duration: 0.6, ease: "power3" }); function icko(e) { xTo(e.clientX); yTo(e.clientY); cursorY = e.clientY }
         window.addEventListener("mousemove", icko); function defaultHover(targets) {
            $(targets).on('mouseenter', function () { mouseCursor.addClass('hover-size') })
            $(targets).on('mouseleave', function () { mouseCursor.removeClass('hover-size'); })
         }
         defaultHover('a , .menu-toggle, .rotate-text-area , .project-slide-button, button, .hover-classic, .cv-toggle,  .scroll-button'); function textHover(targets) {
            $(targets).on('mouseenter', function () { let $this = $(this), text = $this.data('cursor-text'); mouseCursor.addClass('hover-size'); mouseCursor.addClass('hover-text'); cursorText.children('span').html(text) })
         ...
         function zoomHover(targets) {
            $(targets).on('mouseenter', function (e) { if (!$(targets).hasClass('img-lightbox-open')) { let $this = $(this), winWidth = $(window).outerWidth(); mouseCursor.addClass('hover-size'); mouseCursor.addClass('hover-icon'); cursorIcon.attr('data-icon', 'fullscreen'); } })
   ...
   mouseCursor.hasClass('dark') ? cursorDark = true : ''; mouseCursor.hasClass('light') ? cursorLight = true : ''; function cursorLayoutChange(layout, color, reset) {
      mouseCursor.removeClass('dark light'); if (reset) { cursorDark ? mouseCursor.addClass('dark') : ''; cursorLight ? mouseCursor.addClass('light') : ''; }
   ...
   function pageLoader() {
      if ($('.page-loader').length) {
         let perc = $('.page-loader-percentage'), totImages, loadedImages, percentage, minTime = 5, img = $('.page-loader-logo').children('img'), caption = $('.page-loader-caption'), countLabels = [0], i = 0, loader = $('.page-loader'); loader.append('<div class="page-loader-overlays"></div>'); let overlays = loader.find('.page-loader-overlays'); if (loader.hasClass('columns')) {
            ...
            for (i = 0; i <= 4; i++) { overlays.append('<span class="loader-overlay"></span>'); }
         } else if (loader.hasClass('blocks')) {
            overlays.addClass('blocks')
            for (i = 0; i <= 9; i++) { overlays.append('<span class="loader-block"><span></span></span>'); }
         } else {
            overlays.addClass('columns')
            for (i = 0; i <= 4; i++) { overlays.append('<span class="loader-col"></span>'); }
         }
         ...
         let tl = gsap.timeline({
            delay: .1, id: 'pageLoaderAnimation', onStart: () => { $('html').addClass('loading'); }, onComplete: () => {
               if (winLoaded == true) {
                  setTimeout(function () {
                     if (overlays.hasClass('columns')) {
                        let div = $('.page-loader > div').not('.page-loader-overlays'); innerTl.to(div, { opacity: 0, yPercent: -100, ease: 'power3.in' })
                        innerTl.to('.loader-col', {
                           height: '0%', delay: 0, id: 'loader-end', stagger: { grid: [1, 20], from: "random", amount: .3, }, duration: 1.2, ease: 'expo.out', onStart: () => {
                              let instance = ScrollTrigger.getAll(), i = 0; for (i = 0; i < instance.length; i++) { instance[i].enable(); }
                              ScrollTrigger.refresh(true)
                              $('html').removeClass('loading');
                           }, onComplete: () => { transitions.removeClass('half') }
                        })
                        innerTl.play()
                     } else if (overlays.hasClass('overlay')) {
                        let div = $('.page-loader > div').not('.page-loader-overlays'); innerTl.to(div, { opacity: 0, yPercent: -100, ease: 'power3.in' })
                        innerTl.to('.loader-overlay', {
                           height: '0%', delay: 0, duration: 1, ease: 'expo.inOut', onStart: () => {
                              setTimeout(function () {
                                 let instance = ScrollTrigger.getAll(), i = 0; for (i = 0; i < instance.length; i++) { instance[i].enable(); }
                                 ScrollTrigger.refresh(true)
                                 $('html').removeClass('loading');
                        ...
                        innerTl.to('.loader-block span', {
                           height: '0%', stagger: -0.03, delay: 0, duration: 1, ease: 'power1.out', onStart: () => {
                              let instance = ScrollTrigger.getAll(), i = 0; for (i = 0; i < instance.length; i++) { instance[i].enable(); }
                              ScrollTrigger.refresh(true)
                              $('html').removeClass('loading');
                           }, onComplete: () => { transitions.removeClass('half') }
            ...
            new SplitText(caption, { type: 'lines, chars', linesClass: 'capt_line', charsClass: 'capt_char' })
         ...
         $('body').imagesLoaded().progress(function (instance, image) {
            totImages = instance.images.length; loadedImages = instance.progressedCount; percentage = 100 / (totImages / loadedImages); let snapPerc = gsap.utils.snap(countLabels, percentage); if (perc.length) { let countFind = '.count_' + snapPerc + '', transVal = $(countFind).position().top, calc = 100 / snapPerc, calcWin = $(window).outerHeight() / calc, i = 0, spacing = mobileQuery.matches ? 60 : 150; tl.to('.page-loader-count', { y: -1 * $(countFind).position().top, duration: 1, ease: 'expo.inOut' }, 'label_' + snapPerc); tl.to(perc, { y: -1 * (calcWin - (calcWin != 0 ? perc.outerHeight() : 0) - (calcWin == $(window).outerHeight() ? spacing : 0)), duration: 1, ease: 'expo.inOut' }, 'label_' + snapPerc); }
            if (img.length) {
               let logoMask = 'inset(' + (100 - percentage) + '% 0% 0% 0%)'
               tl.to('img.clone', { clipPath: logoMask }, 'label_' + snapPerc);
            }
            if (caption.length) {
               let capt = caption.find('.capt_line.clone'), wido = percentage + '%'
               tl.to(capt, { width: wido, duration: 1, ease: 'expo.inOut' }, 'label_' + snapPerc);
            }
         }).done(function () { });
      }
   }; var naylaSmoother = false; function smoothScroll() {
      let body = $('body'); if (body.hasClass('smooth-scroll')) {
         naylaSmoother = true; $('#page').wrap('<div id="smooth-wrapper"></div>'); $('main').wrap('<div id="smooth-content"></div>'); if ((!siteHeader.hasClass('fixed')) && (!siteHeader.hasClass('sticky'))) {
   ...
   var headerLight = false, headerDark = true; if (siteHeader.hasClass('light')) {
      headerLight = true
      headerDark = false
   }
   function headerLayoutChange(layout, color, smooth, reset) {
      if (!siteHeader.hasClass('blend')) {
         function resetHeader() {
            if (smooth) { let tl = gsap.timeline({}); tl.to(siteHeader, { opacity: 0, duration: 0.175, ease: 'none', onComplete: () => { siteHeader.removeClass('dark light'); } }, 0); tl.to(siteHeader, { opacity: 1, duration: 0.175, ease: 'none', onStart: () => { headerLight ? siteHeader.addClass('light') : ''; headerDark ? siteHeader.addClass('dark') : ''; } }, 0.2) } else {
   ...
   function naylaDynamicList() {
      let list = $('.nayla-dynamic-list'); list.each(function () {
         let list = $(this), item = list.find('ul').children('li'), image = item.find('.list-image'), animation = list.data('animation'), scrub = list.data('scrub'); animation ? list.find('a').addClass(animation) : ''; scrub ? list.find('a').data('scrub', true) : ''; item.each(function () {
            let $this = $(this); animation ? new naylaTextAnimation($this.find('a')) : ''; $this.find('a').on('mouseenter', () => { list.addClass('hovered'); $this.addClass('current'); })
   ...
   function naylaHeader() {
      var siteHeader = $('.site-header'); var headerHeight = siteHeader.outerHeight(), stickyTargets = siteHeader.find('.hide-sticky'), logo = $('.site-logo'), stickyLogo = $('.sticky-logo'), headerPaddingTop = siteHeader.children('div').css('paddingTop'), headerPaddingBottom = siteHeader.children('div').css('paddingBottom'), resetDelay = 1, fixed, sticky, dynamic; matchMedia.add({ isMobile: "(max-width: 450px)" }, (context) => {
         let { isMobile } = context.conditions; if (siteHeader.hasClass('blend')) { gsap.set(siteHeader, { mixBlendMode: 'exclusion' }) }
         return () => { }
      }); function animateLogos(direction) {
         if (stickyLogo) {
            let tl = gsap.timeline(); if (direction == 'play') {
               tl.to(logo, { opacity: 0, duration: 0.35, onComplete: () => { logo.hide() } })
               tl.fromTo(stickyLogo, { opacity: 0 }, { opacity: 1, duration: 0.35, onStart: () => { stickyLogo.show() } });
            } else if (direction == 'reverse') {
               tl.to(stickyLogo, { opacity: 0, duration: 0.35, onComplete: () => { stickyLogo.hide() } })
      ...
      function resetHeader(stAnim) {
         animateLogos('reverse'); if (dynamic != true) {
            if (sticky) {
               gsap.to(siteHeader, { duration: .6, onComplete: () => { siteHeader.removeClass('sticked'); headerLight ? siteHeader.addClass('light') : siteHeader.removeClass('light'); headerDark ? siteHeader.addClass('dark') : siteHeader.removeClass('dark'); gsap.set(siteHeader, { clearProps: 'all' }) } })
               ...
               if (stickyTargets.length) { gsap.to(stickyTargets, { opacity: 1, yPercent: 0, duration: .6, ease: 'power1.inOut' }) }
            } else if (fixed) { siteHeader.removeClass('sticked'); }
         } else { siteHeader.removeClass('sticked'); }
      }
      var delayedCall = gsap.delayedCall(resetDelay, resetHeader); delayedCall.kill(); function fixedHeader() {
         var fixedStart = 'top top', allInstances = ScrollTrigger.getAll(); allInstances.forEach(function (st) { if ((st.pin != null) && (st.start <= 0)) { fixedStart = 'top+=' + st.end + ' top'; } })
      ...
      function stickyHeader() {
         let stickyStart = 'top+=1000 center', stickyReset = 'top top', topPin = [], smoothSticky; var stickyAnim = ScrollTrigger.create({ trigger: "body", start: 'top top', end: 'bottom bottom', id: 'stickyAnim', onEnter: () => { if (smoothSticky != null) { smoothSticky.disable(); } }, onUpdate: function (self) { if (self.direction == -1) { gsap.to(siteHeader, { y: '0%' }) } else if (self.direction == 1) { gsap.to(siteHeader, { y: '-100%' }) } }, }); var allInstances = ScrollTrigger.getAll(); stickyAnim.disable(); allInstances.forEach(function (st) { if ((st.pin != null) && (st.start <= 0)) { topPin.push(st); } })
         if (topPin.length) {
            stickyStart = 'top+=' + (600 + topPin[0].end) + ' center'; stickyReset = 'top+=' + topPin[0].end + ' top'; if ($('body').hasClass('smooth-scroll')) { let pin = ScrollTrigger.create({ trigger: 'body', pin: siteHeader, start: 'top top', end: stickyReset, scroller: '#smooth-content' }) } else { let pin = ScrollTrigger.create({ trigger: '#page', pin: siteHeader, start: 'top top', end: stickyReset, }) }
            ScrollTrigger.create({
               trigger: 'body', start: stickyReset, end: stickyStart, onLeave: () => {
                  gsap.set(stickyTargets, { opacity: 0, yPercent: -100 })
                  ...
                  stickyAnim.enable(); siteHeader.addClass('sticked'); animateLogos('play')
               }, onLeaveBack: () => { siteHeader.removeClass('sticked'); stickyAnim.disable(); animateLogos('reverse') }, onEnter: () => { delayedCall.kill(); headerLight ? siteHeader.addClass('light') : siteHeader.removeClass('light'); headerDark ? siteHeader.addClass('dark') : siteHeader.removeClass('dark'); }
            })
         } else {
            if ($('body').hasClass('smooth-scroll')) {
               gsap.to(siteHeader.children('div'), { yPercent: -100, scrollTrigger: { trigger: 'body', start: 'top top', end: 'top+=' + siteHeader.outerHeight() + ' top', scrub: true, id: 'smoothSticky' } })
               smoothSticky = ScrollTrigger.getById('smoothSticky');
            }
            ScrollTrigger.create({
               trigger: 'body', start: 'top top', end: 'top+=1000 center', onLeave: () => {
                  gsap.set(stickyTargets, { opacity: 0, yPercent: -100 })
                  ...
                  stickyAnim.enable(); siteHeader.addClass('sticked'); animateLogos('play')
               }, onLeaveBack: (self) => { delayedCall.restart(true); stickyAnim.disable(); if (smoothSticky != null) { smoothSticky.enable(); } }, onEnter: () => { delayedCall.kill(); }
            })
         }
      }
   ...
   function siteNavigation() {
      var resetNavigation = ''; function classicNavigation() {
         let nav = $('#site-navigation.classic'), overlay = $('.menu-overlay'), headerHeight = siteHeader.outerHeight(), subBg = nav.data('sub-background'); if (nav.hasClass('one-page')) {
            nav.children('ul').children('li').each(function () {
               let $this = $(this), target = $this.children('a').attr('href'); $this.find('a').on('click', (e) => { e.preventDefault() })
               $this.find('a').addClass('nayla-scroll-button'); $this.find('a').attr('data-scroll-to', target);
            })
         }
         let hasChild = nav.find('.main-menu').children('.nayla-has-children'); var resetHeader = gsap.delayedCall(.7, headerLayoutChange, [false, false, false, true]); resetHeader.kill(); setTimeout(function () {
            hasChild.each(function (i) {
               $(this).addClass('sub_id_' + i); let $this = $(this), classes = $this.attr('class'), arr = classes.split(' '), tltl = gsap.timeline({ id: 'sub_menu_tl_' + i }), x = 0; $this.find('.nayla-sub-menu-wrap').addClass('sub_' + i).insertBefore(siteHeader)
               var subTl = gsap.getById('sub_menu_tl_' + i); for (x = 0; x < arr.length; x++) {
                  if (arr[x].includes('sub_id')) {
                     var subid = arr[x].split('_')[2]; $this.attr('data-sub-id', subid); let sub = '.sub_' + subid, bg = $(sub).css('backgroundColor'), overlay = $(sub).find('.sub-menu-overlay'), ht = $(sub).outerHeight() + siteHeader.outerHeight(), gs = gsap.fromTo($(sub), { height: 0 }, { height: ht, duration: 1, ease: 'power3.inOut', onStart: () => { $(sub).addClass('active'); setTimeout(function () { headerLayoutChange(false, bg, false, false) }, 350) }, onReverseComplete: () => { $(sub).removeClass('active') } }); $(sub).insertBefore(siteHeader)
               ...
               let findSub = '.sub_' + $this.data('sub-id'); $(findSub).on('mouseenter', function () {
                  let $this = $(this), bg = $this.children('div').children('.elementor-section').css('backgroundColor'); cursorLayoutChange(false, bg, false)
                  subTl.pause(); resetHeader.kill()
               })
               $(findSub).on('mouseleave', function () { subTl.reverse(true); resetHeader.restart(true); cursorLayoutChange(false, false, true) })
            })
         }, 90)
      ...
      classicNavigation(); function menuItemAnims(menu, direction) {
         menu.each(function () {
            let items = $(this).children('li'), targets = [items.children('a').find('.mt_char'), items.children('.sub-toggle')], val = items.outerHeight(), stg = 0.02; $(this).hasClass('sub-menu') ? stg = 0.01 : ''; if ((direction === 'in') && (targets.length > 0)) { gsap.fromTo(targets, { y: val }, { y: 0, stagger: stg, duration: 1.5, ease: 'expo.out', overwrite: true, onStart: () => { $(this).addClass('active'); }, }) } else if ((direction === 'out') && (targets.length > 0)) {
               var animOut = gsap.fromTo(targets, { y: 0 }, {
                  y: -1 * val, stagger: 0.005, overwrite: true, duration: .5, ease: 'expo.in', onStart: () => { $(this).addClass('animating') }, onComplete: () => {
      ...
      function fullscreenNavigation() {
         let nav = $('#site-navigation.fullscreen_menu'), wrapper = nav.children('.fullscreen-menu-wrapper'), mainMenu = nav.find('.main-menu'), headerHeight = $('.site-header').outerHeight(), items = mainMenu.find('li a'), hasChildren = mainMenu.find('li.menu-item-has-children'); items.addClass('menu-link'); if (nav.length) {
            hasChildren.each(function () { let childLength = $(this).find('li').not('.sub-menu-title').length; $(this).append('<span class="sub-toggle">(' + childLength + ')</span>'); })
            ...
            wrapper.css('paddingTop', headerHeight + 50); hasChildren.children('.sub-toggle').on('click', function () {
               let $this = $(this).parent('li'), parentMenu = $this.parent('ul'), subMenu = $this.children('.sub-menu-wrapper').children('.sub-menu'); subClose.addClass('active')
      ...
      fullscreenNavigation(); function menuToggle(targetMenu) {
         var menuToggle = $('.menu-toggle'), overlay = $('.menu-overlay'), text = menuToggle.find('.toggle-text'), closeText = text.attr('data-close-text'), bgColor = $('#site-navigation.fullscreen_menu').css('backgroundColor'); text.wrapInner('<span data-close-text="' + closeText + '"></span>'); var headerReset = gsap.delayedCall(1, headerLayoutChange, [false, false, false, true]); headerReset.kill(); setTimeout(function () {
            let navTweens = gsap.getTweensOf(targetMenu.find('*').not('.main-menu *, .nayla-button, .nayla-button *')), i = 0, tltl = gsap.timeline({ id: 'fs_menu_tl' }); if (navTweens.length > 0) {
               for (i = 0; i < navTweens.length; i++) {
                  if (navTweens[i]) {
                     if (navTweens[i].parent) {
                        navTweens[i].parent.pause()
                        navTweens[i].parent.scrollTrigger !== null ? navTweens[i].parent.scrollTrigger.kill() : ''; tltl.add(navTweens[i], (0 + i / 10))
                     } else {
                        navTweens[i].pause()
                        navTweens[i].scrollTrigger !== null ? navTweens[i].scrollTrigger.kill() : ''; tltl.add(navTweens[i], (0 + i / 10))
                     }
                  }
               }
            } else { return; }
            tltl.pause(); menuToggle.on('click', function () {
               var clicks = $(this).data('clicks'), mainMenu = targetMenu.find('.menu.main-menu'); if (clicks) {
                  enableScroll(); let activeMenu = targetMenu.find('ul.menu.active, ul.sub-menu.active'); targetMenu.find('.sub-close').removeClass('active'); $('html').hasClass('loading') ? '' : headerReset.restart(true); menuItemAnims(activeMenu, 'out'); tltl.timeScale(3)
                  tltl.reverse(); gsap.killTweensOf(overlay); gsap.to(targetMenu, { height: 0, duration: .7, delay: .5, ease: 'expo.inOut', onComplete: () => { menuToggle.removeClass('active'); targetMenu.removeClass('active'); siteHeader.removeClass('menu-open'); } })
               } else {
                  disableScroll(); menuToggle.addClass('active')
   ...
   class naylaGeneralAnimations {
      DOM = { el: null, }; animations = ['fadeIn', 'fadeUp', 'fadeDown', 'fadeLeft', 'fadeRight', 'scaleIn', 'slideUp', 'slideLeft', 'slideRight', 'maskUp', 'maskDown', 'maskLeft', 'maskRight', 'maskCustom']; defaults = { duration: 1, delay: 0, stagger: 0, ease: 'power1.inOut', }; from = {}
      ...
      constructor(DOM_el, options, fromOptions, scroll, out) {
         ...
         if (activeAnimation[0] === 'scaleIn') {
            this.target.attr('data-start') == null ? this.from.scale = 0 : this.from.scale = this.target.attr('data-start'); this.target.css('transformOrigin', this.target.attr('data-origin'))
         ...
         if ((activeAnimation[0] === 'slideRight') || (activeAnimation[0] === 'slideLeft')) {
            let leftVal = this.target.offset().left + this.target.outerWidth(), rightVal = $(window).outerWidth() - leftVal + this.target.outerWidth(); activeAnimation[0] === 'slideRight' ? this.from.x = -1 * leftVal : this.from.x = rightVal; this.defaults.x = 0
         ...
         if ((activeAnimation[0] === 'slideUp')) {
            this.from.y = $(window).outerHeight(); this.scroll.scrollTrigger.trigger = this.target[0].offsetParent; this.defaults.y = 0
            this.defaults.duration = 2
   ...
   }; function naylaGeneralAnims(target) { target.each(function () { let $this = $(this); new naylaGeneralAnimations($this) }) }
   function naylaParallaxImages() {
      var image = $('div.single-image.parallax-image'); image.each(function () {
         let $this = $(this), img = $this.find('img'), width = $this.outerWidth(), height = $this.outerHeight(), speed = $this.data('parallax-speed') == null ? 60 : $this.data('parallax-speed'), parallaxVal = height * (speed / 100), imgHeight = 'calc(100% + ' + parallaxVal + 'px)'; $this.wrapInner('<div class="image-parallax-wrap"></div>'); let wrap = $this.find('.image-parallax-wrap'); $this.css('height', height); wrap.css('height', imgHeight)
   ...
   class naylaImageAnimation {
      ...
      constructor(DOM_el, options, fromOptions, scroll, out) {
         ...
         if ((activeAnimation[0] === 'blockUp') || (activeAnimation[0] === 'blockDown') || (activeAnimation[0] === 'blockLeft') || (activeAnimation[0] === 'blockRight')) {
            $(this.DOM.el).append('<span class="block-ov"></div>'); this.target = $(this.DOM.el).find('.block-ov'); let bg = $(this.DOM.el).data('block-color'); if (bg === 'auto') {
               let imgSrc = $(this.DOM.el).find('img').attr('src')
               colorjs.average(imgSrc, { amount: 1 }).then(imgColor => { $(this.DOM.el).find('.block-ov').css('backgroundColor', "rgb" + "(" + imgColor + ")"); })
            } else { $(this.DOM.el).find('.block-ov').css('backgroundColor', $(this.DOM.el).data('block-color')); }
         ...
         this.delay == null ? this.delay = this.defaults.delay : ''; this.duration == null ? this.duration = this.defaults.duration : ''; this.scrub == null ? this.scrub = false : ''; this.options = Object.assign(this.defaults, options); this.fromOptions = Object.assign(this.from, fromOptions); this.scroll = Object.assign(this.scroll, scroll); if (this.pin) {
            this.scrub = true
            this.scroll.scrollTrigger.pin = true; this.scroll.scrollTrigger.scrub = 1; this.scroll.scrollTrigger.start = 'center center'; this.scroll.scrollTrigger.end = this.duration == null ? 'botom top' : 'bottom+=' + (this.duration * 1000) + ' top';
         }
   ...
   class naylaTextAnimation {
      ...
      constructor(DOM_el, options, fromOptions, scroll, out) {
         ...
         if (activeAnimation[0] === 'charsRight') {
            let arr = []; $(this.DOM.el).find('.anim_char').each(function () { arr.push($(this).outerWidth()) })
         ...
         if (activeAnimation[0] === 'linesMask') {
            $(this.DOM.el).find('.anim_line').each(function () { let $this = $(this), span = $this.children('span'); span.clone().addClass('clone').appendTo($this); })
            this.from.width = '0%'
         ...
         if (activeAnimation[0] === 'slideLeft') {
            this.target = $(this.DOM.el).find('.anim_line'); this.from.x = -1 * this.target.outerWidth(); if (this.target.outerWidth() > $(window).outerWidth()) { this.defaults.x = 0 } else { this.defaults.x = 0; }
         ...
         if (activeAnimation[0] === 'slideRight') {
            this.target = $(this.DOM.el).find('.anim_line'); this.from.x = $(window).outerWidth(); if (this.target.outerWidth() > $(window).outerWidth()) { this.defaults.x = 0 - (this.target.outerWidth() - $(window).outerWidth() + 300) } else { this.defaults.x = 0; }
            this.animOut ? this.defaults.x = -1 * $(window).outerWidth() - 400 : ''; this.animOut ? this.out.ease = 'none' : ''; this.defaults.ease = 'none';
         }
         this.stagger == null ? this.stagger = this.defaults.stagger : ''; this.delay == null ? this.delay = this.defaults.delay : ''; this.duration == null ? this.duration = this.defaults.duration : ''; this.options = Object.assign(this.defaults, options); this.fromOptions = Object.assign(this.from, fromOptions); this.scroll = Object.assign(this.scroll, scroll); this.options.stagger = this.stagger; this.options.delay = this.delay; this.options.duration = this.duration; if (this.pin) {
            this.scrub = true
      ...
      render() {
         ...
         this.tl.fromTo(this.target, this.fromOptions, this.options); this.animOut == true ? this.tl.to(this.target, this.out) : ''; this.tl.eventCallback("onComplete", () => {
            let loader = gsap.getById("innerTl"); $(this.DOM.el).addClass('is-inview')
            this.scrub != true && !this.noRevert && loader == null ? this.split.revert() : ''; this.progress = 1;
         }); this.tl.eventCallback("onStart", () => { $(this.DOM.el).addClass('anim_start') });
      }
   }; function naylaTextAnims() {
      let hasAnim = $('.has-anim-text'), exclude = 'ul, li, a'; hasAnim.not(exclude).each(function () {
         let $this = $(this)
         let textAnim = new naylaTextAnimation($this); matchMedia.add({ isMobile: "(max-width: 450px)" }, (context) => {
            if (textAnim.progress == 0) { textAnim.split.revert(); textAnim.tl.revert(); new naylaTextAnimation($this); }
            return () => { }
         }); if ($this.hasClass('wordsJustifyCollapse') || $this.hasClass('wordsJustifyExpand')) {
            let words = $this.find('.anim_word'), start = $this.offset().top < $(window).outerHeight() ? 0 : 'top bottom+=200', scrub = $this.data('scrub')
   ...
   function naylaTextWrapper() {
      var twrap = $('.text-wrapper'); twrap.each(function () {
         let $scope = $(this); var bump = $scope.find('.bump'), turn = $scope.find('.turn'), rotateX = $scope.find('.rotateX'), rotateY = $scope.find('.rotateY'); if (bump.length) {
         ...
         if ($scope.find('.slide-right').length) {
            let sRight = $scope.find('.slide-right'); sRight.wrapInner('<span><span>')
         ...
         if ($scope.find('.slide-left').length) {
            let sLeft = $scope.find('.slide-left'); sLeft.wrapInner('<span><span>')
   ...
   function naylaSinglePostPage() {
      var singlePost = $('.single-post-page'); singlePost.each(function () {
         let post = $(this), meta = post.find('.entry-meta'), metaWrap = post.find('.entry-meta-wrap'), trigEnd = "bottom bottom-=" + (metaWrap.outerHeight() / 2 - 75) + "'", content = post.find('.entry-content'), image = post.find('.post-featured-image'), title = post.find('.post-title-sub'); new SplitText(title, { type: 'words, lines', wordsClass: 'pt-sub-word' })
         ScrollTrigger.create({
            trigger: content, start: 'top top+=50', end: trigEnd, pin: meta, onEnter: () => {
               gsap.fromTo('.pt-sub-word', { yPercent: 100 }, { yPercent: 0, stagger: 0.025, duration: 1, ease: 'expo.out' })
               gsap.to(image, { scale: .8, marginTop: '1em' })
            }, onLeaveBack: () => { gsap.to(image, { scale: 1, marginTop: 0 }) }
         })
      })
   }
   function naylaProductPage() {
      var productPage = $('.product-page-content'), gallery = productPage.find('.product-gallery'), info = productPage.find('.product-info'), endo = gallery.outerHeight() - info.outerHeight(); var galleryScroll = ScrollTrigger.create({ trigger: gallery, pin: info, start: 'top top', end: endo, })
      function detailTabs() {
         let tabs = $('.tab-titles-wrap'), titles = tabs.find('.tab-title'); titles.on('click', function () {
            let $this = $(this), findContent = '.tab_' + $this.attr('data-tab'); $('.tab-content').removeClass('active'); $('.tab-title').removeClass('active'); $(findContent).addClass('active')
            $this.addClass('active')
         })
      }
      detailTabs()
      matchMedia.add({ isMobile: "(max-width: 450px)" }, (context) => {
         let { isMobile } = context.conditions; galleryScroll.kill()
   ...
   function naylaProjectPages() {
      let $this = $('.project-page'), header = $this.find('.project-page-header'), wrapper = header.children('div'), image = $this.find('.project-image.featured'), title = $this.find('.project-title'), backButton = $this.find('.project-back-button'); backButton.on('click', function () { history.back() })
      ...
      if (header.hasClass('image-gallery')) {
         ...
         initAnim()
         if (horizontal) {
            let words = title.find('.anim_word'), state = Flip.getState(words), tl = gsap.timeline({ scrollTrigger: { trigger: header, start: 0, scrub: 1, end: 250, } })
            ...
            slideButton.on('click', () => { gsap.to(window, { duration: .45, scrollTo: 250, ease: 'expo.inOut', }); })
         } else if (vertical) {
            slideButton.on('click', function () {
               let scrollToo = 750 * Math.ceil(ScrollTrigger.getById('projectImagesScroll').progress * 10); if (scrollToo > transVal) { scrollToo = transVal; }
            ...
            matchMedia.add({ isMobile: "(max-width: 450px)" }, (context) => {
               let { isMobile } = context.conditions; initAnim()
   ...
   function naylaPortfolio() {
      let portfolio = $('.portfolio-grid'); portfolio.each(function () {
         ...
         function createSeperators(lengths) {
            portfolio.find('hr').remove(); if (!portfolio.hasClass('masonry')) {
               let line = '<hr></hr>', i = 1; projectsWrapper.find('.grid-project:visible').each(function (i) {
                  i++
         ...
         let switchGrid = portfolio.find('.switch-grid'), switchList = portfolio.find('.switch-list'); switchList.on('click', () => {
            switchList.addClass('active'); switchGrid.removeClass('active'); gsap.to(portfolio.find('hr'), { opacity: 0, })
            gsap.to(projects, {
               clipPath: 'inset(0% 0% 100% 0%)', y: -25, duration: .75, ease: 'circ.inOut', onComplete: () => {
                  ...
                  projects.each(function (i) {
                     let delay = 0 + (i / 8)
                     gsap.fromTo($(this).find('.grid-project-meta > *'), { y: 50 }, {
                        y: 0, duration: 1.5, ease: 'expo.out', delay: delay, stagger: 0.04, onStart: () => { portfolio.addClass('animating'); $(this).addClass('gl-inview'); gsap.set(projects, { clearProps: 'clipPath' }) }, onComplete: () => {
                           portfolio.removeClass('animating'); ScrollTrigger.refresh(true)
            ...
            switchGrid.addClass('active'); switchList.removeClass('active'); portfolio.addClass('animating'); projects.removeClass('gl-inview'); gsap.to(portfolio.find('.grid-project-meta > *'), {
               opacity: 0, duration: .75, ease: 'expo.out', onComplete: () => {
                  gsap.set('.grid-project-meta > *', { clearProps: 'all' })
   ...
   function naylaShopPage() {
      let products = $('.nayla-products-grid'); products.each(function () {
         let products = $(this), grid = products.find('.grid--products'), product = grid.find('.grid--product'), controls = products.find('.product-grid-controls'), length = controls.find('.products--length'), switcher = controls.find('.grid--switcher'), switch4 = switcher.find('.switch-col-4'), switch2 = switcher.find('.switch-col-2'); length.prepend('<span>' + product.length + ' </span>'); switch2.on('click', function () {
            gsap.to(grid, {
   ...
   function slideNav(slider, parent) {
      let prev = parent.find('.slide-prev'), next = parent.find('.slide-next'); next.on('click', function () { slider.slideNext(); })
      prev.on('click', function () { slider.slidePrev(); })
   }
   function showcaseWall() {
      var wallWrap = $('.showcase-wall'); wallWrap.each(function () {
         let mainWrap = $(this), wrapper = mainWrap.find('.showcase-wall-wrap,  .showcase-footer'), overlay = mainWrap.find('.sw-overlay'), project = mainWrap.find('.showcase-project'), title = project.find('.project-title'), projectWraps = mainWrap.find('.sw-projects-wrap'), height = mainWrap.outerHeight(), speed = mainWrap.data('speed'), end = speed == null ? height + 1000 : height + speed, tl = gsap.timeline({ scrollTrigger: { trigger: mainWrap, scrub: 1, pin: true, start: 'top top', end: end, id: 'showcaseWallScroll' } }); if (mainWrap.hasClass('animate-in')) {
            disableScroll(); mainWrap.addClass('animating'); new SplitText(title, { type: 'chars', charsClass: 'pt_char' })
         ...
         projectWraps.each(function () { let $this = $(this), startPos = $this.data('start-pos'), endPos = $this.data('end-pos'); tl.fromTo($this, { xPercent: startPos, }, { xPercent: endPos }, 0) }); var layoutReset = gsap.delayedCall(0.1, layoutChange, [overlay, false, wrapper], { oncomplete: () => { wrapper.removeClass('dark light') } }); layoutReset.kill(); project.each(function (i) {
            i++
            ...
            $this.find('a').on('mouseenter', function () {
               layoutReset.kill(); mainWrap.addClass('hovered'); $this.addClass('current'); image.addClass('transition--media')
         ...
         matchMedia.add({ isMobile: "(max-width: 550px)" }, (context) => {
            let { isMobile } = context.conditions; tl.revert()
            return () => { }
         });
      })
   }
   function showcase3dTitles() {
      let showcase3dTitles = $('.showcase-3d-titles'); showcase3dTitles.each(function () {
         let $this = $(this), projectsWrap = $this.find('.s3t-projects-wrap'), project = $this.find('.showcase-project'), imagesWrap = $this.find('.s3t-images'), fontSize = parseInt($this.find('.project-title').css('font-size')); $this.on('mousemove', function (e) {
            let xTo = gsap.quickTo(imagesWrap, "left", { duration: 0.6, ease: "power3" }), yTo = gsap.quickTo(imagesWrap, "top", { duration: 0.6, ease: "power3" }); function icko(e) { xTo(e.pageX); yTo(e.pageY - $this.offset().top); }
            icko(e)
         }); project.first().addClass('active'); let projectLength = project.length, rotateX = 360 / projectLength; project.each(function (i) {
            let $this = $(this), image = $this.find('.project-image'); $this.find('a').addClass('sc-link')
            image.addClass('image_' + i)
            ...
            let count = 0; $this.find('.project-title').on('mouseenter', () => {
               imagesWrap.addClass('active'); if ($this.hasClass('active')) {
                  imagesWrap.addClass('active'); let activeImage = $('.image_' + i).parent('.d-img'); activeImage.addClass('active')
            ...
            $this.find('.project-title').on('mouseleave', () => {
               imagesWrap.removeClass('active'); count++
         ...
         $this.css('--transformZ', projectLength * fontSize / 6.4 + "px"); let dsc = ScrollTrigger.create({ trigger: $this, start: 'top top', end: 10000, pin: true, scrub: true, })
         matchMedia.add({ isMobile: "(max-width: 550px)" }, (context) => {
            let { isMobile } = context.conditions; $('.project-title span').off('mousemove')
            return () => { $('.project-title span').on('mousemove') }
         }); const lenis = new Lenis({ infinite: true, smooth: true, infinite: true, smoothTouch: false, duration: 2 }); window.addEventListener('scroll', function () {
            gsap.set(projectsWrap, { rotateX: this.scrollY / 10000 * 360 })
   ...
   function showcaseFullscreenWall() {
      var main = $('.showcase-fullscreen-wall'); main.each(function () {
         let parent = $(this), wrapper = parent.find('.sfw-projects-wrapper'), projects = wrapper.find('.showcase-project'), switcList = parent.find('.switch-titles'), switchImages = parent.find('.switch-images'), animation = parent.data('animation'); projects.each(function (i) {
            i++
   ...
   function showcaseList() {
      var sflWrap = $('.showcase-list'); sflWrap.each(function () {
         let $this = $(this), projectsWrap = $this.find('.sfl-projects-wrap'), speed = $this.attr('data-speed'), wrapHeight = projectsWrap.outerHeight(), end = speed == null ? wrapHeight + 1000 : (wrapHeight + parseInt(speed)), transVal = $(window).outerHeight() - wrapHeight, project = $this.find('.showcase-project'), overlay = $this.find('.sc-overlay'); project.each(function (i) {
            i++
            ...
            i >= 10 ? $this.attr('data-index', i) : $this.attr('data-index', '0' + i); image.addClass('image_' + i); $this.on('mouseenter', function (e) {
               let mouseLeft = e.clientX, myLeft = mouseLeft - $this.offset().left; project.not($this).addClass('op-down')
               $this.addClass('hovered')
               $this.find('.project-image').addClass('transition--media')
               gsap.set($this, { zIndex: 2 }); gsap.fromTo(image, { scale: .5, opacity: .5 }, { scale: 1, opacity: 1 })
               gsap.set(image, { left: myLeft, rotate: event.movementX / 7, })
            }); $this.on('mousemove', function (e) { let mouseLeft = e.clientX, myLeft = mouseLeft - $this.offset().left, bright = 'brightness(' + event.movementX + ')'; gsap.to(image, { left: myLeft, rotate: event.movementX / 7, duration: 1, ease: 'power3.out', }) })
   ...
   function showcaseFullscreenSlideshow() {
      var fullscreenSlideshow = $('.showcase-fullscreen-slideshow'); fullscreenSlideshow.each(function () {
         let fsSlideshow = $(this), projects = fsSlideshow.find('.showcase-project'), images = fsSlideshow.find('.fs-images-slider'), footer = fsSlideshow.find('.showcase-footer'), current = fsSlideshow.find('.sfs-current'), total = fsSlideshow.find('.sfs-total'), headerChange, footerChange; images.addClass('transition--media')
         current.html('01')
         ...
         projects.each(function (i) {
            let $this = $(this), image = $this.find('.project-image'), layout = $this.data('layout'), elements = $this.find('.project-inner, .project-button a'), title = $this.find('.project-title'), cat = $this.find('.project-category'), summary = $this.find('.project-summary'); $this.addClass('wrapper'); new SplitText([cat, summary], { type: 'words', wordsClass: 'pd_word' })
         ...
         function headerMainChange(color, delay1, delay2) {
            let hsl = gsap.utils.splitColor(color, true), lightness = hsl[hsl.length - 1]; setTimeout(function () {
            ...
            speed: 1000, slidesPerView: 1, navigaton: { prevEl: '.fs-prev', nextEl: '.fs-next' }, init: false, edgeSwipeDetection: true, mousewheel: { eventsTarget: '.showcase-fullscreen-slideshow', }, direction: 'vetical', parallax: true, on: {
               slideChange: function () {
                  let swiper = this, projectIn = fsSlideshow.find('.project_' + swiper.activeIndex), projectOut = fsSlideshow.find('.project_' + swiper.previousIndex), outMetas = projectOut.find('.pd_word span'), outTitle = projectOut.find('.pt_char span'), outButton = projectOut.find('.project-button span'), inButton = projectIn.find('.project-button span'), inMetas = projectIn.find('.pd_word span'), inTitle = projectIn.find('.pt_char span'); projectIn.addClass('active'); projectOut.removeClass('active'); let bgaColor = projectIn.data('elements-color'); if (swiper.activeIndex > swiper.previousIndex) {
                     ...
                     gsap.from(inButton, { yPercent: 100, xPercent: -100, ease: 'power3.out', duration: .6, stagger: 0.03, delay: 1 })
                  } else {
                     ...
                     gsap.from(inMetas, { yPercent: -100, ease: 'power3.out', duration: .6, stagger: -0.03, delay: 1 })
                     gsap.from(inButton, { yPercent: 100, xPercent: -100, ease: 'power3.out', duration: .6, stagger: 0.03, delay: 1 })
                  }; let activeProj = fsSlideshow.find('.showcase-project.active'), bgColor = activeProj.data('background-color');
               }, progress: function () { let swiper = this; for (let i = 0; i < swiper.slides.length; i++) { let slideProgress = swiper.slides[i].progress, innerOffset = swiper.height * interleaveOffset, innerTranslate = slideProgress * innerOffset; swiper.slides[i].querySelector(".slide-bgimg").style.transform = "translateY(" + innerTranslate + "px)"; } }, setTransition: function (speed) { let swiper = this; for (let i = 0; i < swiper.slides.length; i++) { swiper.slides[i].style.transition = speed + "ms"; swiper.slides[i].querySelector(".slide-bgimg").style.transition = 1000 + "ms"; } }, slideChangeTransitionEnd: function () {
                  let swiper = this; let link = $('.showcase-project.active a').attr('href'); $('.project-button > a').attr('href', link)
                  ...
                  if (($('body').innerHeight()) > ($(window).outerHeight())) { if (swiper.slides.length == (swiper.activeIndex + 1)) { swiper.mousewheel.disable(); ScrollTrigger.create({ trigger: fsSlideshow, start: 'top top', end: 'center top', onLeave: function () { }, onLeaveBack: (self) => { setTimeout(function () { self.kill(); swiper.mousewheel.enable(); }, 1000) } }) } }
               }
            }
         }); slideNav(imagesSwiper, fsSlideshow)
         $(document).ready(function () { imagesSwiper.init(); })
         if (fsSlideshow.hasClass('animate-in')) {
            let tl = gsap.timeline({ once: true, delay: 1 })
   ...
   function layoutChange(target, project, parent) {
      if (project) {
         let primaryColor = project.data('primary-color'), secondaryColor = project.data('secondary-color'), tl = gsap.timeline(); if (primaryColor) { tl.to(target, { backgroundColor: primaryColor, duration: .75, ease: 'sine.out', onStart: () => { headerLayoutChange(false, primaryColor, false, false); cursorLayoutChange(false, primaryColor, false); let hsl = gsap.utils.splitColor(primaryColor, true), lightness = hsl[hsl.length - 1]; if (parent) { lightness < 50 ? parent.addClass('light') : parent.removeClass('light'); lightness > 50 ? parent.addClass('dark') : parent.removeClass('dark'); } } }, 0) } else { tl.to(target, { backgroundColor: 'transparent', duration: .75, ease: 'sine.out', onStart: () => { headerLayoutChange(false, false, false, true); cursorLayoutChange(false, false, true) } }, 0); parent ? parent.removeAttr('style') : ''; }
         if (parent) { if (secondaryColor) { parent.attr('style', '--mainColor: ' + secondaryColor + '') } else { parent.removeAttr('style'); parent.removeClass('dark light') } }
      } else { gsap.to(target, { backgroundColor: 'transparent', duration: .75, ease: 'sine.out', onStart: () => { headerLayoutChange(false, false, false, true); cursorLayoutChange(false, false, true) } }); parent.removeAttr('style'); parent.removeClass('dark light') }
   }
   function projectColorSettings(project) {
      let $this = project, img = project.find('img'), imgSrc = img.attr('src'), bgColor = project.data('primary-color'), elementsColor = project.data('secondary-color'); if ((bgColor == null) && (img.length)) { colorjs.average(imgSrc, { amount: 1 }).then(imgColor => { let finalColor = "rgb" + "(" + imgColor + ")"; $this.attr('data-primary-color', finalColor); }) }
   ...
   function showcaseMinimalList() {
      var mainWrap = $('.showcase-minimal-list'); mainWrap.each(function () {
         var mainWrap = $(this), projectsWrap = mainWrap.find('.sml-projects-wrap'), pwHeight = projectsWrap.outerHeight(), project = projectsWrap.find('.showcase-project'), projectsLength = project.length, images = mainWrap.find('.sml-images'), overlay = mainWrap.find('.sfc-overlay'), animation = mainWrap.data('animation'), speed = mainWrap.data('speed'), colorAnim; let startY = 0; if (mainWrap.hasClass('infinity')) {
            ...
            let prevHeight = mainWrap.find('.clone-prev').length * mainWrap.find('.clone-prev').innerHeight(), cloneLength = (window.innerHeight / 2) / prevHeight; if (cloneLength >= 1) {
               let e = 0
         ...
         project.each(function (i) {
            let $this = $(this), title = $this.find('.project-title'), image = $this.find('.project-image'); $this.find('a').addClass('showcase-project-link'); if (colorAnim == true) { projectColorSettings($this); }
         ...
         window.addEventListener('scroll', function () {
            let progY = (scrollY / speed) * (project.outerHeight(true) * projectsLength); gsap.set(projectsWrap, { y: startY - progY })
   ...
   function showcaseCarousel() {
      var mainWrap = $('.showcase-carousel'); mainWrap.each(function () {
         let mainWrap = $(this), overlay = mainWrap.find('.sfc-overlay'), images = mainWrap.find('.sc-images'), scWrap = mainWrap.find('.sc-wrapper'), wrapperWidth = scWrap.outerWidth(), projectsWrap = mainWrap.find('.sc-projects-wrap'), projects = projectsWrap.find('.showcase-project'), wrapperHeight = mainWrap.outerHeight(), speed = wrapperHeight + mainWrap.data('speed'), frac = mainWrap.find('.slides-fraction'), curr = frac.find('.slide-current'), tot = frac.find('.slide-total'), colorAnim; curr.html('1')
         tot.html(projects.length)
         mainWrap.hasClass('animate-colors') ? colorAnim = true : colorAnim = false; images.addClass('swiper-container').wrapInner('<div class="swiper-wrapper"></div>'); projects.each(function (i) {
            let $this = $(this), image = $this.find('.project-image'); $this.find('a').addClass('sc-link')
         ...
         if (colorAnim) {
            let bgColor = $('.project_0').data('primary-color'); gsap.set(overlay, { backgroundColor: bgColor })
         ...
         }); let firstPos = ($(window).outerWidth() / 2) - ($('.project_0').outerWidth() / 2) + 75, fillLine = mainWrap.find('.sc-line-fill'), lastPos = (wrapperWidth * -1) + ($(window).outerWidth() / 2); tl.to(fillLine, { width: '100%' }, 0)
         tl.fromTo(scWrap, { x: firstPos }, { x: lastPos, ease: 'none', id: 'willUpdated' }, 0)
         matchMedia.add({ isMobile: "(max-width: 550px)" }, (context) => {
            let { isMobile } = context.conditions; tl.revert(); window.scrollTo(0, 0)
            let wrapperWidth = scWrap.outerWidth(), firstPos = ($(window).outerWidth() / 2) - ($('.project_0').outerWidth() / 2) + 75, fillLine = mainWrap.find('.sc-line-fill'), lastPos = (wrapperWidth * -1) + ($(window).outerWidth() / 2), proga = 0; projects.removeClass('active'); mainWrap.find('.project_0').addClass('active'); $('.project-image.swiper-slide').removeClass('transition--media'); $('.image_0').addClass('transition--media'); let scDrag = Draggable.create(scWrap, {
               type: 'x', bounds: { minX: 0, maxX: lastPos }, lockAxis: true, trigger: '.showcase-carousel', dragResistance: 0.5, inertia: true, onThrowUpdate: () => {
                  let prog = Math.floor((scDrag[0].endX * 100) / lastPos) / 100; if (prog <= 0) { projects.removeClass('active'); mainWrap.find('.project_0').addClass('active'); let activeProject = mainWrap.find('.showcase-project.active'), slideGo = activeProject.data('index'); $('.project-image.swiper-slide').removeClass('transition--media'); $('.image_' + slideGo).addClass('transition--media'); } else if (prog >= 0) { projects.each(function (i) { let $this = $(this), ofLeft = $this.offset().left, windowWidth = $(window).outerWidth(), deadLine = (windowWidth / 2) - $this.outerWidth(), currentTransform = scWrap.css('transform'); if ((ofLeft < (windowWidth / 2)) && (ofLeft > deadLine)) { $this.addClass('active'); } else { $this.removeClass('active'); } }); }
                  let activeProject = mainWrap.find('.showcase-project.active'), slideGo = activeProject.data('index'); $('.project-image.swiper-slide').removeClass('transition--media'); $('.image_' + slideGo).addClass('transition--media'); imagesSlider.slideTo(slideGo, 700);
               }, zIndexBoost: false, onDrag: () => {
                  let prog = Math.floor((scDrag[0].endX * 100) / lastPos) / 100; if (prog <= 0.1) { projects.removeClass('active'); mainWrap.find('.project_0').addClass('active'); } else if (prog >= 0) { projects.each(function (i) { let $this = $(this), ofLeft = $this.offset().left, windowWidth = $(window).outerWidth(), deadLine = (windowWidth / 2) - $this.outerWidth(), currentTransform = scWrap.css('transform'); if ((ofLeft < (windowWidth / 2)) && (ofLeft > deadLine)) { $this.addClass('active'); } else { $this.removeClass('active'); } }); }
                  let activeProject = mainWrap.find('.showcase-project.active'), slideGo = activeProject.data('index'); $('.project-image.swiper-slide').removeClass('transition--media'); $('.image_' + slideGo).addClass('transition--media'); curr.html(slideGo + 1)
                  imagesSlider.slideTo(slideGo, 700);
               }
            }); return () => {
               let wrapperHeight = mainWrap.outerHeight(), speed = wrapperHeight + mainWrap.data('speed'), wrapperWidth = scWrap.outerWidth(), firstPos = ($(window).outerWidth() / 2) - ($('.project_0').outerWidth() / 2) + 75, fillLine = mainWrap.find('.sc-line-fill'), lastPos = (wrapperWidth * -1) + ($(window).outerWidth() / 2); tl.to(fillLine, { width: '100%' }, 0)
               tl.fromTo(scWrap, { x: firstPos }, { x: lastPos, ease: 'none', }, 0); ScrollTrigger.getById('showcaseCarouselScroll').vars.end = lastPos * -1
            }
         }); function scIntro() {
            let intro = gsap.timeline({ once: true, scrollTrigger: { start: 'top-=1 top' } }); projects.each(function () {
               let $this = $(this), title = $this.find('.project-title'); var splitTitle = new SplitText(title, { type: 'lines, chars', linesClass: 'pt_line', charsClass: 'pt_char', display: 'inline-block', reduceWhiteSpace: false })
   ...
   function showcaseFullscreenCarousel() {
      var showcaseFullscreenCarousel = $('.showcase-fullscreen-carousel'); showcaseFullscreenCarousel.each(function (i) {
         i++
         ...
         projects.each(function (i) {
            let $this = $(this), title = $this.find('.project-title'), image = $this.find('.project-image'); colorAnim ? projectColorSettings($this) : ''; $this.attr('data-index', i)
         ...
         var titlesSlider = new Swiper('.fc-slideshow-wrap', { slidesPerView: 'auto', centeredSlides: true, speed: 1500, noSwiping: true, slideToClickedSlide: true, allowTouchMove: false, mousewheel: { invert: false, eventsTarget: '.dragging-class' } }), interleaveOffset = 0.5, imagesSlider = new Swiper('.fc-images-slider', {
            slidesPerView: 1, speed: 1500, parallax: true, noSwiping: true, watchSlideProgress: true, on: {
               slideChange: function () {
                  projects.removeClass('active'); gsap.set(dragger, { x: (this.snapGrid[this.activeIndex - 1] * -1) / projLength })
                  if (colorAnim == true) { let act = projectsWrap.find('.project_' + this.activeIndex); layoutChange(overlay, act, projectsWrap); $('.fs-parallax-wrap').removeClass('transition--media'); $('.s-img_' + this.activeIndex).find('.fs-parallax-wrap').addClass('transition--media'); let category = act.find('.project-category').text(); gsap.to(cat, { duration: 1.5, text: { value: category, }, ease: "expo.out" }); }
               }, slideChangeTransitionEnd: function () { let index = $('.showcase-project.swiper-slide-active').data('index') + 1; curr.html(index > 9 ? index : '0' + index) }, progress: function () { let swiper = this; for (let i = 0; i < swiper.slides.length; i++) { let slideProgress = swiper.slides[i].progress, innerOffset = swiper.width * interleaveOffset, innerTranslate = slideProgress * innerOffset; swiper.slides[i].querySelector(".slide-bgimg").style.transform = "translateX(" + innerTranslate + "px)"; } }, setTransition: function (speed) { let swiper = this; for (let i = 0; i < swiper.slides.length; i++) { swiper.slides[i].style.transition = speed + "ms"; swiper.slides[i].querySelector(".slide-bgimg").style.transition = 1500 + "ms"; } },
            }
         }); function checkActiveProject() { projects.each(function () { let $this = $(this), projWidth = Math.PI * $this.outerWidth(), projLeft = Math.PI * $this.offset().left, activeLine = (Math.PI * $(window).outerWidth()) / 2, deadLine = activeLine - (projWidth * 0.5), index = $this.data('index'), bgColor = $this.data('background-color'), elementsColor = $this.data('elements-color'); if ((projLeft < activeLine) && (projLeft > deadLine)) { $this.addClass('active'); imagesSlider.slideTo(index, 1500); $('.fs-parallax-wrap').removeClass('transition--media'); $('.s-img_' + this.activeIndex).find('.fs-parallax-wrap').addClass('transition--media'); } else { $this.removeClass('active') } }) }
         ...
         var titlesDrag = Draggable.create(dragger, {
            type: 'x', bounds: { minX: 0, maxX: draggerEnd / projLength }, lockAxis: true, inertia: false, zIndexBoost: false, dragResistence: 1, onPress: self => {
               titlesSlider.disable(); mainWrap.addClass('dragging'); $('.swiper-slide-active').addClass('active'); gsap.set(projectsWrap.find('.swiper-wrapper'), { clearProps: 'transition-duration' }); if (colorAnim == true) { let project = projectsWrap.find('.showcase-project.swiper-slide-active'); layoutChange(overlay, project, projectsWrap) }
               gsap.to([siteHeader, mainWrap.find('.showcase-footer, .sfc-fraction')], { opacity: 0, duration: .3 })
            }, onDrag: self => {
               let currentTransform = dragger.css('transform'), gimmeThat = currentTransform.split(" ")[4].replace(/\,/g, ''), myVal = gimmeThat * projLength; gsap.to(projectsWrap.find('.swiper-wrapper'), { x: myVal, })
               ...
               gsap.ticker.add(checkActiveProject); if (colorAnim == true) { let project = projectsWrap.find('.showcase-project.active'); layoutChange(overlay, project, projectsWrap) }
            }, onRelease: self => {
               gsap.ticker.remove(checkActiveProject); mainWrap.removeClass('dragging'); let activeProj = projectsWrap.find('.showcase-project.active').data('index'); gsap.to([siteHeader, mainWrap.find('.showcase-footer, .sfc-fraction')], { opacity: 1, duration: .5, delay: .3 })
               gsap.to(projectsWrap.find('.swiper-wrapper'), {
                  x: snapGrid[activeProj] * -1, onComplete: () => {
                     projects.removeClass('active')
                     titlesSlider.enable(); titlesSlider.slideTo(activeProj, 1500); titlesSlider.update();
                  }
               })
            }
   ...
   function showcaseInfiniteGrid() {
      let fullscreenGrid = $('.showcase-infinite-grid'); fullscreenGrid.each(function (i) {
         i++
         ...
         if (mainWrap.hasClass('horizontal')) {
            horizontal = true; grid.append('<div class="sfg_grid_row sfg_grid_row_top"></div>')
            grid.append('<div class="sfg_grid_row sfg_grid_row_bottom"></div>')
         }
         colorAnim = true; mainWrap.attr('id', id); let parent = document.getElementById(id); const repeatItems = (parentEl, total = 0) => { const items = [...parentEl.children]; for (let i = 0; i <= total - 1; ++i) { var cln = items[i].cloneNode(true); parentEl.appendChild(cln); } }; repeatItems(parent.querySelector('.sfg-grid-wrapper'), cols * 2); let projects = grid.find('.showcase-project'); projects.each(function (i) {
            i++
         ...
         requestAnimationFrame(raf); lenis.on('scroll', (e) => {
            var bgMarquee = gsap.getById('nayla_marquee'), velocity = e.velocity / 15
   ...
   function showcaseInteractiveGrid() {
      let grid = $('.showcase-interactive-grid'); grid.each(function () {
         let grid = $(this), wrapper = grid.children('.projects-wrapper'), projects = wrapper.find('.showcase-project'), animation = grid.data('animation'), positions = []; projects.each(function (i) {
            let $this = $(this), left = $this.offset().left + ($this.offset().left > $(window).outerWidth() / 2 ? 75 : 0), top = $this.offset().top > $(window).outerHeight() / 2 ? $this.offset().top + ($(window).outerHeight() / 4) : $this.offset().top, title = $this.find('.project-title'), details = $this.find('.project-details'), singlePos = []; $this.find('a').addClass('inner-project-link')
         ...
         function zoomOut() {
            let state = Flip.getState(projects, { props: 'opacity, filter' }), tl = gsap.timeline(); projects.removeClass('active'); gsap.set(wrapper, { x: 0, y: 0 })
         ...
         if (grid.hasClass('animate-in')) {
            ...
            ScrollTrigger.create({
               trigger: 'body', start: 'top-=1 top', onEnter: () => {
                  zoomOut()
   ...
   function showcaseCards() {
      let showcaseCards = $('.showcase-cards'); showcaseCards.each(function () {
         let cards = $(this), wrapper = cards.find('.ssv-projects-wrap'), item = cards.find('.showcase-project'), speed = cards.data('speed'), length = item.length, height = item.outerHeight(), scEnd = height * length; item.clone().addClass('clone').appendTo(wrapper)
         item.clone().appendTo(wrapper)
         item = cards.find('.showcase-project'); item.each(function (i) {
            ...
            $(this).attr('data-y', -i * 30); $(this).on('mouseenter', function () {
               $(this).find('.project-image').addClass('transition--media')
               $(this).addClass('active')
            })
         ...
         let sct = new ScrollTrigger.create({ animation: tl, trigger: cards, pin: true, start: 'top top', end: scEnd * 3, scrub: true, }); cards.find('.anim-project').each(function (i) {
            tl.to($(this), {
               ease: 'none', y: $(this).data('y') + (cards.find('.clone').length * 30), z: $(this).data('z') + (cards.find('.clone').length * 100), onUpdate: () => {
                  let cloneItem = cards.find('.clone-project_' + $(this).data('index'))
                  gsap.set(cloneItem, { ease: 'none', y: $(this).css('transform').split(',')['13'] - (cards.find('.clone').length * 30), z: $(this).css('transform').split(',')['14'] - (cards.find('.clone').length * 100), })
               },
            }, 0)
   ...
   function horizontalList() {
      $('.showcase-horizontal-list').each(function () {
         ...
         gsap.to(titlesWrap, {
            xPercent: -100, ease: 'none', scrollTrigger: {
               trigger: mainWrap, scrub: 1, end: 'bottom+=5000 top', pin: true, onUpdate: self => {
                  let findProj = '.project_' + Math.ceil(self.progress * projLength), findImage = '.project_image_' + Math.ceil(self.progress * projLength), activeIndex = $(findProj).data('index'), snapProg = self.progress * projLength; if (activeIndex < 10) { curr.html('0' + activeIndex) } else { curr.html(activeIndex) }
   ...
   var switchedDark, switchedLight; function layoutSwitcher() {
      function logosSwitch() {
         let inverts = $('.ls-invert'), brights = $('.ls-bright'); if (inverts.length) { inverts.find('img').toggleClass('inverted') }
         if (brights.length) { brights.find('img').toggleClass('brighted') }
      }
      let switcher = $('.nayla-layout-switcher'), switchDark = switcher.find('.switch-dark'), switchLight = switcher.find('.switch-light'), follower = switcher.find('.active-follower'), body = $('body'); if (switcher.hasClass('ls_switcher')) {
         body.hasClass('light') ? switchLight.addClass('active') : switchLight.removeClass('active'); body.hasClass('dark') ? switchDark.addClass('active') : switchDark.removeClass('active'); barba.hooks.after((data) => { body.hasClass('light') ? switchLight.addClass('active') : switchLight.removeClass('active'); body.hasClass('dark') ? switchDark.addClass('active') : switchDark.removeClass('active'); }); switchDark.on('click', function () {
            switchLight.removeClass('active'); $(this).addClass('active')
         ...
         switchLight.on('click', function () {
            switchDark.removeClass('active'); $(this).addClass('active')
            ...
            logosSwitch()
            switchedDark = false; switchedLight = true;
         })
      } else if (switcher.hasClass('ls_button')) {
         body.hasClass('light') ? switcher.addClass('light-active') : switcher.removeClass('light-active'); body.hasClass('dark') ? switcher.addClass('dark-active') : switcher.removeClass('dark-active'); barba.hooks.after((data) => { body.hasClass('light') ? switcher.addClass('light-active') : switcher.removeClass('light-active'); body.hasClass('dark') ? switcher.addClass('dark-active') : switcher.removeClass('dark-active'); }); switcher.on('click', function () {
            logosSwitch()
            if (body.hasClass('light')) {
               ...
               switcher.removeClass('light-active'); switcher.addClass('dark-active'); switchedDark = true; switchedLight = false;
   ...
   function postsGrid() {
      var grid = $('.nayla-posts-grid'); grid.each(function () {
         let grid = $(this), filters = grid.find('.grid--filters .filters-list > li'), posts = grid.find('.grid--post--item'), switcher = grid.find('.grid--switcher'); filters.each(function () {
            let $this = $(this), cat = $this.data('category'); $this.on('click', function () {
   ...
   function naylaProductRatings() {
      var rating = $('.product-rating'); rating.each(function () {
         let $this = $(this), stars = $this.find('.product-stars'), perc = $this.data('rating'), i = 1; for (i = 1; i <= 5; i++) { stars.append('<span class="material-icons">star</span>') }
         stars.clone().addClass('stars-fill').appendTo($this); gsap.set($this.find('.stars-fill'), { width: perc + '%' })
      })
   }
   naylaProductRatings()
   function naylaAddTocartButton() {
      var button = $('.product-add-to-cart'); button.each(function () {
         let dcr = $('.count--decrease'), inc = $('.count--increase'), input = $('.current-quantity'), currVal = input.attr('value'), x = 1; dcr.on('click', () => {
            x--
   ...
   function naylaLightboxGallery(gallery, items) {
      let $items = $(items), $gallery = $(gallery), itemsLenght = $items.length, thumbsDragger, thumbsWheeler; const $lightbox = $('<div>', {
         class: 'nayla-lightbox-hold', html: `
                    ...
                    <div class="lightbox-fraction"><div class="lf-curr"></div><div class="lf-tot">${itemsLenght}</div></div>
                       `}), lightboxReset = $lightbox.html(), itemsState = Flip.getState($items, { props: 'padding , opacity, filter' }); function createNav(range, length, index) {
         let wrap = $('.lightbox-convert'), x = 0, result = 0, prevVal = []; for (x = 0; x < index; x++) { prevVal.push($('.thumb_' + x).outerWidth()) }
         ...
         const snaps = $items.map(function () { return -($(this).position().left + ($(this).outerWidth() / 2)); }).get(); function detectActive() {
            $items.each(function () {
               let $this = $(this), entrance = $this.offset().left, deadLine = $this.offset().left + $this.outerWidth(), center = $(window).outerWidth() / 2; entrance < center && center < deadLine ? $this.addClass('active') : $this.removeClass('active'); let active = $('.grid--item.active').data('index'); $('.lightbox-gal img').removeClass('active')
      ...
      $(items).each(function (i) {
         let $this = $(this), img = $this.find('img'), cl = $(items).find('img').clone(); $this.attr('data-index', i); $this.addClass('thumb_' + i); $this.on('click', () => {
            if (!$gallery.hasClass('lightbox-active')) {
               $this.addClass('active'); disableScroll(); gsap.killTweensOf($items); $('body').append($lightbox); $gallery.addClass('lightbox-active'); setTimeout(function () { cl.appendTo('.lightbox-gal'); $('.lightbox-gal').find('img').each(function (i) { $(this).addClass('lb--img--' + i); }); $lightbox.find('.lf-curr').html(i + 1) })
      ...
      function lightboxAnim(direction) {
         let tl = gsap.timeline({ onStart: () => { $('body').css('cursor', 'wait') }, }), lightbox = $lightbox, overlay = lightbox.find('.lightbox-overlay'); if (direction === 'in') { tl.to(overlay, { height: '100%', duration: 1.5, ease: 'expo.out', delay: 1, onComplete: () => { $lightbox.addClass('active'); thumbsDragger[0].enable(); } }) } else if (direction === 'out') {
            tl.to(lightbox, {
               clipPath: 'inset(0% 0% 100% 0%)', duration: 1, ease: 'expo.inOut', onStart: () => { $gallery.css('zIndex', 9999999999999); thumbsDragger[0].disable(); }, onComplete: () => {
   ...
   function naylaDynamicGrid() {
      const grid = $('.nayla-dynamic-grid'); grid.each(function () {
         let grid = $(this), wrapper = grid.children('.grid--wrapper'), items = wrapper.children('.grid--item'), animation = grid.data('animation'); grid.hasClass('lightbox-gallery') ? grid.css('height', grid.outerHeight()) : ''; grid.hasClass('lightbox-gallery') ? naylaLightboxGallery(wrapper, items) : ''; items.each(function (i) {
            i++
            let $this = $(this); $this.find('a').addClass('inner-project-link'); $this.find('.single-project-image').addClass('trans-media'); if (grid.hasClass('parallax-on') && $this.find('img').length) {
               $this.find('img').wrap('<div class="parallax-wrap"></div>'); $this.find('.parallax-wrap').css('width', $this.outerWidth()); $this.find('img').css('height', 'calc(100% + 100px)'); gsap.to($this.find('img'), { y: -100, ease: 'none', scrollTrigger: { trigger: $this, scrub: 1, start: 'top bottom', emd: 'bottom top', } })
               $(window).on('resize', () => {
                  gsap.set($this.find('.parallax-wrap'), { clearProps: 'width' })
                  $this.find('.parallax-wrap').css('width', $this.outerWidth());
               })
            }
            if (animation != null) {
               let stg = 0; grid.attr('data-stagger') < 0 ? stg = -1 * (grid.attr('data-stagger') / i) : stg = grid.attr('data-stagger') * i; $this.addClass(animation); $this.attr('data-duration', grid.attr('data-duration')); $this.attr('data-delay', stg)
   ...
   function naylaFilterableGrid() {
      let $scope = $('.nayla-filterable-grid'), wrapper = $scope.find('.nfg--wrapper'), items = $scope.find('.nfg--item'), filtersWrapper = $scope.find('.nfg--filters--wrapper'), filters = $scope.find('.nfg--filters--wrapper li'); items.each(function (i) {
         i++
      ...
      filters.each(function () {
         let $this = $(this); $this.on('click', function () {
            filters.not($this).removeClass('active'); $this.addClass('active'); let state = Flip.getState(items); gsap.to(window, { scrollTo: wrapper.offset().top - 100, duration: .5 })
   ...
   function naylaForms() {
      let form = $('.nayla-form'); form.each(function () {
         let $this = $(this), inputs = $this.find('input, textarea'); inputs.each(function () {
            let $this = $(this), wrap = $this.parent('div'), label = wrap.find('label'); $this.on('focus', () => { wrap.addClass('active'); })
   ...
   function naylaDynamicCarousel() {
      const carousel = $('.nayla-dynamic-carousel'); carousel.each(function () {
         ...
         function navScroll() {
            let endtrigger = speed == null ? 'bottom+=3000 bottom' : 'bottom+=' + speed + ' bottom', start = customPin == null ? 'center center' : 'top top', pinTarget = customPin == null ? carousel : customPin, endNumber = speed == null ? 1000 : 1000 + speed; if (carousel.hasClass('items-bottom')) { start = 'bottom bottom' }
            wrapScroll = gsap.timeline({ scrollTrigger: { trigger: pinTarget, start: start, end: carousel.offset().top < $(window).outerHeight() ? endNumber : endtrigger, pin: true, id: 'caroScroll', scrub: 1, pinReparent: $('body').hasClass('smooth-scroll') ? true : false, onLeaveBack: () => getCurrentItem(), onEnterBack: () => getCurrentItem(), onScrubComplete: () => getCurrentItem(), onEnter: () => getCurrentItem(), onLeave: () => getCurrentItem(), onUpdate: () => getCurrentItem() } }); wrapScroll.to(wrapper, { x: -end, })
         }
         var snaps; function getSnaps() { snaps = items.map(function () { if (mobileQuery.matches) { return -($(this).position().left + ($(this).outerWidth()) + parentLeft); } else { return -($(this).position().left + ($(this).outerWidth() / 2) + parentLeft); } }).get(); }
         function navPrevNext() {
            next.on('click', function () {
               getSnaps(); let nextProj = wrapper.find('.carousel--item.next'), marg = parseInt(nextProj.css('marginRight')), index = nextProj.data('index') - 1
         ...
         carousel.hasClass('navScroll') ? navScroll() : navDrag(); carousel.find('.carousel--navigation').length ? navPrevNext() : ''; matchMedia.add({ isMobile: "(max-width: 450px)" }, (context) => {
            let { isMobile } = context.conditions; gsap.set(wrapper, { x: 0 })
            ...
            end = wrapper.outerWidth() - items.last().outerWidth(); if (carouselDrag) {
               carouselDrag[0].applyBounds({ minX: 0, maxX: -end })
            ...
            return () => {
               ...
               parentLeft = carousel.offset().left; gsap.set(items.find('img'), { clearProps: 'transform' })
               end = wrapper.outerWidth() - items.last().outerWidth() - (items.last().outerWidth() / 2); if (carouselDrag) {
                  carouselDrag[0].applyBounds({ minX: 0, maxX: -end })
                  carouselDrag[0].update()
               }
               if (wrapScroll) {
   ...
   function naylaParallax() {
      const item = $('.has-parallax'); item.each(function () {
         let $this = $(this), tl = gsap.timeline({ scrollTrigger: { trigger: $this, start: 'top bottom', end: 'bottom top', scrub: 1 } }), speed = $this.data('parallax-speed'), direction = $this.data('parallax-direction'), x = 0, y = 0; speed == null ? speed = 20 : ''; direction == null ? direction = 'up' : ''; direction == 'up' ? y = (speed * -1) : ''; direction == 'down' ? y = speed : ''; direction == 'left' ? x = (speed * -1) : ''; direction == 'right' ? x = speed : ''; tl.to($this, { yPercent: y, xPercent: x, }); matchMedia.add({ isMobile: "(max-width: 450px)" }, (context) => {
            let { isMobile } = context.conditions; tl.clear(); gsap.set($this, { clearProps: 'all' })
   ...
   function naylaLightbox() {
      let lightbox = $('.lightbox'), isSingle = true; lightbox.each(function () {
         let $this = $(this), imgSrc = $this.attr('src'), height = $this.outerHeight(), parent = $this.parent('div'); parent.css('height', height)
         $this.on('click', () => {
            var clicks = $(this).data('clicks'); if (clicks) { enableScroll(); } else { $('body').append('<div class="lightbox-focus"></div>'); disableScroll() }
   ...
   function gridScroll() {
      let pinned = $('.pinned'); pinned.each(function () {
         let $this = $(this), dataPin = $this.attr('data-pin'), height = $this.outerHeight(); let pinnedScroll = ScrollTrigger.create({ trigger: dataPin, pin: $this, pinSpacing: false, start: 'top top+=' + siteHeader.outerHeight() + '', end: 'bottom top+=' + (height + siteHeader.outerHeight()) + '' })
         matchMedia.add({ isMobile: "(max-width: 450px)" }, (context) => {
            pinnedScroll.kill()
            return () => { }
         });
   ...
   function naylaSections() {
      var section = $('#content').find('.section'), page = $('body'), pageBg = $('#page').css('background-color'), pinned; function changeBg(section, color, leave) {
         gsap.to('#page', { backgroundColor: color, duration: .6, onComplete: () => { if (leave) { gsap.set('#page', { clearProps: 'all' }) } } }); section.addClass('active')
      ...
      var video = $(vid), mouseCursor = $('#mouseCursor'); if ((video.length) && (!video.hasClass('vid-ready'))) {
         video.each(function (i) {
            ...
            naylaVid.on('ready', (event) => {
               if ($this.find('video').attr('autoplay')) { naylaVid.play() }
               ...
               if ($this.hasClass('play-cursor')) {
                  $this.on('click', function () {
                     if ($this.hasClass('lightbox-play')) {
                        $this.removeClass('cursor-icon'); $this.removeClass('cursor-text'); mouseCursor.removeClass('hover-size')
               ...
               function lightboxPlay() {
                  ...
                  $('.lightbox-close').on('click', function () {
                     ...
                     if ($this.hasClass('ratio-adjusted')) {
                        let parent = $this.parent('div'); const targetHeight = parent.outerHeight(), parentWidth = parent.outerWidth(), parentHeight = parent.outerHeight(); $this.attr('ratio', naylaVid.ratio); let rat = naylaVid.ratio.split(':')[0] / naylaVid.ratio.split(':')[1], parentRatio = parentWidth / parentHeight; if (parentRatio < rat) {
                           $this.addClass('ratio-adjusted'); function adjustIframeSize() {
                              const aspectRatio = rat; const $iframe = $this.find('.nayla-video-wrap'); const targetWidth = targetHeight * aspectRatio; gsap.set($iframe, { width: targetWidth + 10, height: targetHeight + 10, x: -1 * (targetWidth - parentWidth) / 2 })
               ...
               if ($this.hasClass('play-on-hover')) {
                  naylaVid.pause(); $this.on('mouseenter', () => { naylaVid.play(); })
                  $this.on('mouseleave', () => { naylaVid.pause(); })
               }
               if (naylaVid.autoplay == true) { naylaVid.play(); }
            }); if ($this.parents('.has-anim').length && $this.parents('.project-page').length) {
               setTimeout(function () {
                  let anim = gsap.getTweensOf($this.parents('.has-anim')); anim[0].parent.pause()
                  anim[0].parent.scrollTrigger !== null ? anim[0].parent.scrollTrigger.kill() : ''; naylaVid.once('playing', (event) => { anim[0].parent.play(); });
               }, 2)
            }
   ...
   function naylaCircleText() {
      let circularText = $(".nayla-circular-text"); circularText.each(function (i) {
         let $this = $(this); $this.wrapInner("<div class='circular-text-wrap'></div>")
         ...
         let whaler = Hamster(document.querySelector('body')), wheelDeltaY, currentDeltaY; function createWheelStopListener(element, callback, timeout) {
            var handle = null; var onScroll = function () {
               if (handle) { clearTimeout(handle); }
               handle = setTimeout(callback, timeout || 200);
            }; element.addEventListener('wheel', onScroll); return function () { element.removeEventListener('wheel', onScroll); };
         }
   ...
   function naylaButtons() {
      var button = $('.nayla-button'); button.each(function () {
         let button = $(this), $this = $(this).children('a'), icon = $this.find('.button-icon'), tl = gsap.timeline({ paused: true }), line, animation = $this.data('animation'), stagger = $this.data('stagger'), duration = $this.data('duration'); if (animation) {
   ...
   function naylaMarquee(i) {
      var velocity; let marquee = $('.nayla-marquee'); marquee.each(function () {
         let $this = $(this), text = $this.children('*'), dataDuration = $this.data('duration'), seperator = $this.data('seperator'); text.append('<span class="material-icons">' + seperator + '</span>')
         $this.wrapInner('<div class="marquee-wrap"></div>')
         let infItem = $this.find('.marquee-wrap'), infWidht = infItem.outerWidth(), infLenght = window.innerWidth / infWidht, gap = infItem.offset().left; function infinityOnResize() {
            for (i = 2; i < infLenght + 2; i++) { infItem.clone().appendTo($this) }
         ...
         infinityOnResize(); window.onresize = function () {
            $this.find('.nayla-marquee:first-child').nextAll().remove()
            let infItem = $this.find('.infinity-item'), infWidht = infItem.outerWidth(), infLenght = window.innerWidth / infWidht; infinityOnResize();
         }
      })
   }
   function naylaServices() {
      let naylaServices = $('.nayla-services'); naylaServices.each(function () {
         let services = $(this), servicesItem = services.find('.nayla-service'); servicesItem.append("<span class='services-item-bottom-line nayla-service-line'></span>")
   ...
   function naylaAccordion() {
      let naylaAccordion = $('.nayla-accordion'); naylaAccordion.each(function () {
         let $this = $(this), acrdTitle = $this.find('.accordion-title'), acrdContent = $this.find('.accordion-content'), activeContent = $this.find('.active > .accordion-content'), icon = $this.data('toggle-icon'), animation = $this.data('animation'); if (animation) {
         ...
         acrdTitle.on('click', function () {
            ...
            if (acrdItem.hasClass('active')) { acrdItem.removeClass('active') } else {
   ...
   function naylaClinetsCarousel() {
      let clientCarousel = $('.nayla-clients-carousel'); clientCarousel.each(function () {
         let $this = $(this), clientWrap = $this.find('.clients-wrapper'), clientFullWidth = clientWrap.outerWidth(), dataDuration = $this.data('duration'), windowWidth = window.innerWidth, tl = gsap.timeline(); if (clientFullWidth < windowWidth) {
            let i = 0, clientLenght = Math.ceil(windowWidth * 2 / clientFullWidth); for (i = 1; i <= clientLenght; i++) { clientWrap.clone().appendTo($this) }
         ...
         function dragMove() {
            var x = parseInt($this.css('transform').split(',')[4]), speed = clientFullWidth / dataDuration
            if ($this.hasClass('left-to-right')) {
               gsap.fromTo($this, { x: x }, {
                  x: 0, duration: (-1 * x) / speed, ease: 'none', onComplete: () => {
                     gsap.set($this, { x: 0 })
                     tl.fromTo($this, { x: -1 * clientFullWidth }, { x: 0, duration: dataDuration, ease: 'none', repeat: -1 })
                  }
               })
            } else {
               gsap.fromTo($this, { x: x }, {
                  x: -1 * clientFullWidth, ease: 'none', duration: (clientFullWidth - (-1 * x)) / speed, onComplete: () => {
                     gsap.set($this, { x: 0 })
   ...
   function naylaInfiniteCarousel() {
      let clientCarousel = $('.nayla-infinite-carousel'); clientCarousel.each(function () {
         let $this = $(this), clientWrap = $this.find('.carousel--wrapper'), clientFullWidth = clientWrap.outerWidth(), dataDuration = $this.data('duration'), windowWidth = window.innerWidth, tl = gsap.timeline(); if (clientFullWidth < windowWidth) {
            let i = 0, clientLenght = Math.ceil(windowWidth * 2 / clientFullWidth); for (i = 1; i <= clientLenght; i++) { clientWrap.clone().appendTo($this) }
            $this.css('width', (clientLenght + 1) * clientFullWidth)
         } else {
            clientWrap.clone().appendTo($this)
   ...
   function naylaTestimonials() {
      let testimonials = $('.nayla-testimonials'); testimonials.each(function () {
         ...
         wrap.css('height', single.first().outerHeight()); single.first().addClass('active'); let tl = gsap.timeline({ repeat: -1 }); single.each(function (i) {
            let $this = $(this), circle = $this.find('svg circle'); $this.addClass('testimonial_' + i)
            ...
            tl.from(circle, 5, { duration: 2, drawSVG: 0, onComplete: () => { gsap.set(circle, { drawSVG: 0 }) } }, 'label' + i); if ($this.next().length) {
               tl.to($this.next(), { opacity: 1, duration: 0.5, delay: 6, onStart: () => { current.html($this.next().data('index') + 1 > 10 ? $this.next().data('index') + 1 : '0' + ($this.next().data('index') + 1)) } }, 'label' + i)
               tl.to(wrap, { height: $this.next().outerHeight(), duration: 0.5, delay: 5.9, }, 'label' + i)
            } else { tl.to($('.testimonial_0'), { opacity: 1, duration: 0.5, delay: 6, onStart: () => { current.html('01') } }, 'label' + i) }
         })
         nav.on('mouseenter', () => { tl.pause() })
         nav.on('mouseleave', () => { tl.resume() })
         let clicksForward = 0, clicksBackward = single.length; next.on('click', () => {
            clicksForward++
            ...
            gsap.to(wrap, {
               opacity: 0, onComplete: () => {
                  let tlSeek = 'label' + clicksForward; tl.seek(tlSeek)
         ...
         prev.on('click', () => {
            ...
            gsap.to(wrap, {
               opacity: 0, onComplete: () => {
                  let tlSeek = 'label' + clicksBackward; tl.seek(tlSeek)
                  gsap.to(wrap, { opacity: 1 })
               }
            })
         })
   ...
   const noise = () => {
      let canvas, ctx; let wWidth, wHeight; let noiseData = []; let frame = 0; let loopTimeout; const createNoise = () => {
         const idata = ctx.createImageData(wWidth, wHeight); const buffer32 = new Uint32Array(idata.data.buffer); const len = buffer32.length; for (let i = 0; i < len; i++) { if (Math.random() < 0.5) { buffer32[i] = 0xff000000; } }
         noiseData.push(idata);
      }; const paintNoise = () => {
         if (frame === 9) { frame = 0; } else { frame++; }
         ctx.putImageData(noiseData[frame], 0, 0);
      }; const loop = () => { paintNoise(frame); loopTimeout = window.setTimeout(() => { window.requestAnimationFrame(loop); }, (1000 / 25)); }; const setup = () => {
         wWidth = window.innerWidth; wHeight = window.innerHeight; canvas.width = wWidth; canvas.height = wHeight; for (let i = 0; i < 10; i++) { createNoise(); }
         loop();
      }; let resizeThrottle; const reset = () => { window.addEventListener('resize', () => { window.clearTimeout(resizeThrottle); resizeThrottle = window.setTimeout(() => { window.clearTimeout(loopTimeout); setup(); }, 200); }, false); }; const init = (() => { canvas = document.getElementById('noised-bg'); ctx = canvas.getContext('2d'); setup(); })();
   }; $('#noised-bg').length ? noise() : ''; function naylaScrollButtons() {
   ...
   function naylaNumberCounter() {
      let numberCt = $('.nayla-number-counter'); numberCt.each(function () {
         let $this = $(this), ctNumberSplit = new SplitText($this.find('.ct-number'), { type: 'words, chars', charsClass: 'ct-number-char', wordsClass: 'ct-number-word' }), ctChar = $this.find('.ct-number-char'), ctWord = $this.find('.ct-number-word'), dataCount = $this.data('count'), ctStart = $this.find('.count-start'), ctEnd = $this.find('.count-end'); ctChar.each(function () {
            let $this = $(this); $this.wrapInner("<div class='value-loop-char'></div>")
   ...
   function naylaTeamMember() {
      const member = $('.nayla-team-member'); member.each(function () {
         let $this = $(this), cv = $this.find('.team-member-cv'), card = $this.find('.team-member-card'), image = $this.find('.team-member-image'), parentHeight = $this.outerHeight(), cardHeight = card.outerHeight(), cvHeight = cv.outerHeight(); image.append('<div class="cv-toggle"><span class="material-icons">add</span></div>')
         let cvToggle = $this.find('.cv-toggle'); cvToggle.on('click', function () {
            var clicks = $(this).data('clicks'), tl = gsap.timeline(); if (clicks) {
               $this.removeClass('active'); tl.to(cv, { opacity: 0, duration: .5, display: 'none', onComplete: () => { gsap.set(card, { paddingBottom: cvHeight + 50, }) } })
               if ($this.hasClass('overlay')) { tl.to(card, { paddingBottom: 0, }, 0.5) } else {
                  tl.to(image, { height: 'auto', duration: .75, ease: 'expo.out' }, 0.5)
                  tl.to(card, { paddingBottom: 0, duration: .75, ease: 'expo.out' }, 0.5)
               }
            } else {
               $this.addClass('active'); if ($this.hasClass('overlay')) { tl.to(card, { paddingBottom: cvHeight + 50, }) } else {
                  tl.to(image, { height: parentHeight - (cardHeight + cvHeight + 50 + 15), duration: 1, ease: 'expo.out' })
   ...
   function naylaInfiniteTabs() {
      let naylaInfiniteTab = $('.nayla-infinite-tabs'); naylaInfiniteTab.each(function () {
         let $this = $(this), tabTitleWrap = $this.find('.tab-title-wrap'), tabTitle = $this.find('.tab-title'); $this.find('.tab-content').each(function (i) { i++; $(this).addClass('data-index_' + i) })
         ...
         tabTitle.on('click', function () {
            ...
            gsap.to(tabTitleWrap, {
               x: -1 * prevWidth, duration: 1.5, ease: 'expo.out', onStart: () => {
                  let tl = gsap.timeline()
                  tl.to($this.find('.tab-content'), {
                     opacity: 0, duration: $this.data('duration') / 2, onComplete: () => {
                  ...
                  gsap.to($this.find('.tab-contents-wrap'), { height: $this.find('.tab-content.data-index_' + $(this).data('index')).outerHeight(true) })
               }, onComplete: () => {
   ...
   function initPageElements() {
      ...
      naylaCartButton()
      naylaTeamMember()
      naylaCircleText(); naylaButtons(); naylaMarquee()
      naylaServices()
      naylaNumberCounter(); naylaAccordion(); naylaInfiniteTabs(); naylaClinetsCarousel(); naylaInfiniteCarousel(); naylaDynamicGrid(); naylaFilterableGrid(); naylaForms(); gridScroll()
      naylaTestimonials()
      naylaDynamicCarousel(); naylaParallax(); naylaLightbox()
      naylaDynamicList(); detectPov()
      circleMaskBg(); naylaSingleProject();
   }
   function initShowcases() {
      showcaseWall(); showcaseList(); showcaseFullscreenSlideshow(); showcaseMinimalList(); showcaseCarousel(); showcaseInfiniteGrid(); showcaseFullscreenCarousel(); horizontalList(); showcaseFullscreenWall(); showcaseInteractiveGrid()
      showcaseCards(); showcase3dTitles()
   }
   if (($('.page-loader').length) && (!sessionStorage.getItem('doNotShow'))) { sessionStorage.setItem('doNotShow', 'true'); pageLoader() } else { $('.page-loader').remove() }
   naylaMouseCursor(true); smoothScroll(); $(window).on('load', function () {
      winLoaded = true; window.scrollTo(0, 0); naylaVideo($('.nayla-video')); naylaGeneralAnims($('.has-anim')); naylaTextAnims(); naylaTextWrapper(); naylaListAnimations(); naylaImageAnims()
      naylaParallaxImages(); initShowcases(); initPageElements(); naylaSections(); initPages(); naylaHeader(); siteNavigation(); naylaScrollButtons(); layoutSwitcher(); ScrollTrigger.refresh(true); let loader = gsap.getById("innerTl"); if (loader != null) { setTimeout(function () { let instance = ScrollTrigger.getAll(), i = 0; for (i = 0; i < instance.length; i++) { instance[i].disable(); } }, 3) }
   })
   ...
   let chars = $('.capt_chars'), words = $('.capt_words'), lines = $('.capt_lines'); chars.wrapInner('<span></span>'); function transitionCaption(tl, intro, outro) {
      if (transitions.hasClass('default')) {
         if (intro) { tl.fromTo(chars.find('span'), { y: 100 }, { y: 0, duration: 1, ease: 'power3.out', stagger: 0.01, }, .3) }
         if (outro) { tl.to(chars.find('span'), { y: -100, duration: .6, stagger: -0.01, ease: 'power3.in', }, .3) }
      } else if (transitions.hasClass('offset-x')) {
         if (intro) { tl.fromTo(chars.find('span'), { x: 50 }, { x: 0, duration: .6, ease: 'power3.out', stagger: 0.02, }, .5) }
         if (outro) { tl.to(chars.find('span'), { x: -50, duration: .6, stagger: 0.02, ease: 'power3.in', }, .3) }
      } else if (transitions.hasClass('opacity')) {
         if (intro) { tl.fromTo(chars, { opacity: 0 }, { opacity: 1, duration: .6, ease: 'power3.out', stagger: 0.02, }, 0.3) }
   ...
   function animateOverlay(time, intro, outro) {
      if (intro) { time.to(transitions.find('.transition-overlay'), { height: '100%', width: '100%', duration: 1, ease: 'expo.inOut', onComplete: () => { transitions.addClass('half') } }, 0) }
   ...
   function animateColumns(time, intro, outro) {
      if (intro) { transitions.css('visibility', 'visible'); if (left || right) { time.to('.trans-col', { width: '100%', stagger: { grid: [1, 20], from: "random", amount: .3, }, duration: .8, ease: 'expo.out', onComplete: () => { transitions.addClass('half') } }) } else { time.to('.trans-col', { height: '100%', stagger: { grid: [1, 20], from: "random", amount: .3, }, duration: .8, ease: 'expo.out', onComplete: () => { transitions.addClass('half') } }) } }
   ...
   function animateOpacity(time, intro, outro) {
      if (intro) { time.to('#primary, .project-page-header', { y: up ? -100 : down ? 100 : 0, x: left ? 100 : right ? -100 : 0, opacity: 0, duration: 1, ease: 'power3.in', }, 0) }
   ...
   function animatePageOver(time, intro, outro) {
      let origin = $(window).scrollTop(), brightOverlay = '<span class="bright-ov"></span>'; gsap.set('#content , .project-page-header', { transformOrigin: 'center ' + origin })
      if (intro) {
         time.to(transitions, { height: '100vh', width: '100vw', duration: 1.5, ease: 'expo.inOut' }, 0)
         time.to('.page-over-ovs', { opacity: 1, scale: 1, duration: 1, }, 0)
      }
      if (outro) {
         gsap.set('.page-over-ovs', { clearProps: 'all' })
         time.to(transitions, { opacity: 0, duration: 1 }, 0)
      }
   }
   function getMediaToCenter(tl, duration) {
      var media = $('.transition--media'); if (!media.find('.plyr__video-embed__container').length) { media.clone().addClass('clone').insertAfter(media); }
   ...
   function animateMedia(tl, fullscreen, slide, mask, direction) {
      var media = document.querySelector('.transition--media'); if (fullscreen) { tl.to(media, { width: '100vw', height: '100vh', duration: 1, ease: 'expo.inOut', }) }
   ...
   function projectBackground(current, next, tl, delay, smooth) {
      if (current === next) { } else {
         ...
         tl.to(ov, {
            height: '100svh', duration: 1, ease: 'expo.inOut', onStart: () => {
               if (smooth) {
                  setTimeout(function () { gsap.to(siteHeader.children('div').children('section'), { opacity: 0, duration: .3 }) }, 650)
                  setTimeout(function () { gsap.to(siteHeader.children('div').children('section'), { opacity: 1, duration: .3 }) }, 1200)
               } else { setTimeout(function () { headerLayoutChange(false, next, false, false) }, 650) }
            }
         }, delay)
      }
   }
   function fullscreenCarouselOut(tl) {
      let slides = $('.showcase-fullscreen-carousel .showcase-project.swiper-slide').not('.swiper-slide-active'), title = $('.showcase-project.swiper-slide-active').find('.project-title'); new SplitText(title, { type: 'chars', charsClass: 'fc_char' })
   ...
   function fullscreenSlideshowOut(tl) {
      let active = $('.fs-slideshow-wrap .showcase-project.active'); tl.to([active.find('.pt_char'), active.find('.pd_word span')], { yPercent: -100, duration: .75, ease: 'expo.in', stagger: 0.02 }, 0)
   ...
   function outAnimShowcaseCarousel(time) {
      let carousel = $('.showcase-carousel'), project = carousel.find('.showcase-project.active'), title = carousel.find('.project-title'); time.to(title, { yPercent: -100, duration: 1, ease: 'expo.inOut' }, 0)
   ...
   function showcaseCardsOut(tl) {
      let items = $('.showcase-project'), active = $('.showcase-project.active'); gsap.killTweensOf(items)
   ...
   function fullscreenWallOut(tl) {
      let targets = $('main').find('*').not('trans-image'); tl.to('.title_line', { yPercent: -25, rotateX: -90, duration: 1, ease: 'expo.in' }, 0)
      tl.to('.project-title, .project-category, .showcase-footer', { opacity: 0, duration: 1, ease: 'expo.in' }, 0)
   }
   function showcase3dTitlesOut(tl) {
      let currRot = $('.s3t-projects-wrap').css('transform'); tl.to('.s3t-projects-wrap .project-title', { y: -200, duration: 1, ease: 'expo.in', opacity: 0, onComplete: () => { $('.showcase-3d-titles').addClass('animatingg'); } }, 0)
      tl.to('.showcase-footer', { opacity: 0 }, 0)
   }
   function showcaseImageLeave(tl, media) {
      let medTop = media.offset().top - $(window).scrollTop(), targets = $('main').find('.elementor-widget-container'), exclude = media.parents('.elementor-widget-container').first(), others = exclude.find('.single-project-meta'), wido = media.outerWidth(), height = media.outerHeight(), left = media.offset().left; if (media.find('.n-vimeo').length || media.find('.n-youtube').length) { } else {
         gsap.set(media, { top: medTop, width: wido, height: height, left: left, position: 'fixed', zIndex: 999 })
   ...
   function projectImageLeave(tl, media) {
      ...
      if (media.find('.n-vimeo').length || media.find('.n-youtube').length) {
         let targets = $('main').find('.elementor-widget-container'), exclude = media.parents('.elementor-widget-container').first(); tl.to(targets.not(exclude), { opacity: 0, duration: .4 })
         media.addClass('animating')
      } else {
         $('body').hasClass('smooth-scroll') ? media.insertBefore('#smooth-content') : media.insertBefore('main'); media.addClass('animating')
   ...
   function projectImageEnter(tl, response, targetBg) {
      ...
      if (transImage.find('.n-vimeo').length || transImage.find('.n-youtube').length) { tl.to(transImage, { clipPath: 'inset(0% 0% 100% 0%)', duration: 1, ease: 'expo.inOut', }, 1) } else {
         let state = Flip.getState(transImage); gsap.set(transImage, { position: 'fixed', top: top, height: height, width: width, left: left, y: 0, x: 0, yPercent: 0, xPercent: 0 })
   ...
   function showcaseImageEnter(tl, response, targetBg) {
      ...
      if (transImage.find('.n-vimeo').length || transImage.find('.n-youtube').length) { tl.to(transImage, { clipPath: 'inset(0% 0% 100% 0%)', duration: 1, ease: 'expo.inOut', }, 1) } else {
         let state = Flip.getState(transImage); gsap.set(transImage, { position: 'fixed', top: top, height: height, width: width, left: left, x: 0, y: 0, xPercent: 0, yPercent: 0 })
         let imgTo = Flip.from(state, { duration: 1.2, ease: 'expo.inOut', })
         tl.add(imgTo, 0);
      }
   }
   barba.init({
      debug: true, timeout: 30000, transitions: [{
         name: 'default-transition', leave() {
            return new Promise(function (resolve, reject) {
               transitions.addClass('running')
               let tl = gsap.timeline({ onStart: () => { gsap.set(transitions, { visibility: 'visible' }) }, onComplete: () => { resolve(); } }); block ? animateBlocks(tl, true, false) : overlay ? animateOverlay(tl, true, false) : opacity ? animateOpacity(tl, true, false) : pageOver ? animatePageOver(tl, true, false) : columns ? animateColumns(tl, true, false) : ''; transitionCaption(tl, true, false)
            })
         }, beforeEnter() {
            return new Promise(function (resolve, reject) {
               let tl = gsap.timeline({
                  onStart: () => { resolve(); transitions.removeClass('running'); }, onComplete: () => {
                     gsap.set(transitions, { clearProps: 'all' })
                     if (block) { gsap.set('.transition-block', { clearProps: 'all' }) }
                  }
               }); block ? animateBlocks(tl, false, true) : overlay ? animateOverlay(tl, false, true) : opacity ? animateOpacity(tl, false, true) : pageOver ? animatePageOver(tl, false, true) : columns ? animateColumns(tl, false, true) : ''; transitionCaption(tl, false, true)
            })
         }
      }, {
         name: 'fullscreen-menu-transition', from: { custom: ({ trigger }) => { return trigger.classList && trigger.classList.contains('menu-link'); }, }, leave() {
            return new Promise(function (resolve, reject) {
               transitions.addClass('running')
               ...
               block ? animateBlocks(tl, true, false) : overlay ? animateOverlay(tl, true, false) : opacity ? animateOpacity(tl, true, false) : pageOver ? animatePageOver(tl, true, false) : columns ? animateColumns(tl, true, false) : ''; transitionCaption(tl, true, false)
            })
         }, beforeEnter() { return new Promise(function (resolve, reject) { let tl = gsap.timeline({ onStart: () => { resolve(); transitions.removeClass('running') }, onComplete: () => { gsap.set(transitions, { clearProps: 'all' }) } }); block ? animateBlocks(tl, false, true) : overlay ? animateOverlay(tl, false, true) : opacity ? animateOpacity(tl, false, true) : pageOver ? animatePageOver(tl, false, true) : columns ? animateColumns(tl, false, true) : ''; transitionCaption(tl, false, true) }) }
      }, {
         name: 'media-center-to-image', from: { custom: ({ trigger }) => { return trigger.classList && trigger.classList.contains('sc-link'); }, }, to: { namespace: ['project-half-image', 'project-image-gallery-horizontal', 'project-no-image', 'project-image-gallery-vertical', 'project-video', 'project-creative', 'project-boxed-image', 'project-fullscreen-image', 'project-tall-image'] }, leave() {
            return new Promise(function (resolve, reject) {
         ...
         name: 'inline-project-transition', from: { custom: ({ trigger }) => { return trigger.classList && trigger.classList.contains('inner-project-link'); }, }, to: { namespace: ['project-boxed-image', 'project-fullscreen-image', 'project-half-image', 'project-creative', 'project-tall-image', 'project-image-gallery-horizontal', 'project-image-gallery-vertical', 'project-video'] }, leave(trigger) {
            return new Promise(function (resolve, reject) {
               let clicked = $(trigger.trigger); let tl = gsap.timeline({ onComplete: () => { resolve(); } }), media; if (clicked.find('.trans-media').length) { media = clicked.find('.trans-media'); } else { media = $('.trans-media') }
         ...
         name: 'showcase-project-transition', from: { custom: ({ trigger }) => { return trigger.classList && trigger.classList.contains('showcase-project-link'); }, }, to: { namespace: ['project-boxed-image', 'project-fullscreen-image', 'project-half-image', 'project-creative', 'project-tall-image', 'project-image-gallery-horizontal', 'project-image-gallery-vertical', 'project-video'] }, leave() {
            return new Promise(function (resolve, reject) {
               let tl = gsap.timeline({ onComplete: () => { resolve(); } })
         ...
         name: 'projects-grid-transition', from: { custom: ({ trigger }) => { return trigger.classList && trigger.classList.contains('grid-project-link'); }, }, to: { namespace: ['project-boxed-image', 'project-fullscreen-image', 'project-half-image', 'project-creative', 'project-tall-image', 'project-image-gallery-horizontal', 'project-image-gallery-vertical', 'project-video'] }, leave(trigger) {
            return new Promise(function (resolve, reject) {
               let tl = gsap.timeline({ onComplete: () => { resolve(); } })
               ...
               tl.to(window, {
                  scrollTo: media.offset().top - ($(window).outerHeight() - media.outerHeight()) + (($(window).outerHeight() - media.outerHeight()) / 2), duration: 2, ease: 'power3.out', onComplete: () => {
               ...
               }); let otherProj = $('.grid-project').not(media.parents()), targets = $('main').find('.elementor-widget-container'), exclude = media.parents('.elementor-widget-container').first(), lastTar; if (targets.length > 1) { lastTar = targets.not(exclude) } else { lastTar = null }
            ...
            return new Promise(function (resolve, reject) {
               ...
               if (bodyClasses.includes("dark")) { targetBg = darkBg }
               let tl = gsap.timeline({
                  onStart: () => { gsap.set($('main').last(), { opacity: 0 }) }, onComplete: () => {
                     resolve(); gsap.set($('main').last(), { opacity: 1 })
         ...
         name: 'next-project-transition', from: { custom: ({ trigger }) => { return trigger.classList && trigger.classList.contains('next-project-link'); }, }, to: { namespace: ['project-boxed-image', 'project-fullscreen-image', 'project-half-image', 'project-creative', 'project-tall-image', 'project-image-gallery-horizontal', 'project-image-gallery-vertical', 'project-video'] }, leave() {
            return new Promise(function (resolve, reject) {
               let tl = gsap.timeline({ onComplete: () => { resolve(); } })
               ...
               tl.to(window, {
                  scrollTo: parent.offset().top, duration: 1, ease: 'power3.out', onComplete: () => {
   ...
   barba.hooks.before((data) => { startLoading(); }); barba.hooks.enter((data) => {
      var response = data.next.html.replace(/(<\/?)body( .+?)?>/gi, '$1notbody$2>', data.next.html), bodyClasses = $(response).filter('notbody').attr('class'), bodyStyles = $(response).filter('notbody').attr('style'), headerClasses = $(response).filter('notbody').find('#header').attr('class'), headerInner = $(response).filter('notbody').find('#header').html(); var $newPageHead = $('<head />').html($.parseHTML(data.next.html.match(/<head[^>]*>([\s\S.]*)<\/head>/i)[0], document, true)); var headTags = ['meta[name="keywords"]', 'meta[name="description"]', 'meta[property^="og"]', 'meta[name^="twitter"]', 'meta[itemprop]', 'link[itemprop]', 'link[rel="prev"]', 'link[rel="next"]', 'link[rel="canonical"]', 'link[rel="alternate"]', 'link[rel="shortlink"]', 'link[id*="google-fonts"]', 'style[id*="nayla-body-styles"]'].join(','); $(response).remove(); $('head').find(headTags).remove(); $newPageHead.find(headTags).appendTo('head'); if ($('body').hasClass('smooth-scroll')) { let aliothSmoother = ScrollSmoother.get(); aliothSmoother.scrollTo(0, false); window.scrollTo(0, 0); } else { window.scrollTo(0, 0); }
      gsap.set('#page', { clearProps: 'all' })
      $('body').attr('class', bodyClasses); if (switchedDark == true) {
         $('body').addClass('dark')
      ...
      naylaSmoother == true ? $('body').addClass('smooth-scroll') : ''; $('body').attr('style', bodyStyles); $('.site-header').attr('class', headerClasses); if (!$('.site-header').hasClass('blend')) {
         if (switchedDark == true) {
            headerLight = true; headerDark = false; $('.site-header').removeClass('dark')
            $('.site-header').addClass('light')
         } else if (switchedLight == true) {
            headerLight = false; headerDark = true; $('.site-header').removeClass('light')
      ...
      naylaParallaxImages(); initShowcases(); initPageElements(); naylaSections(); initPages(); naylaVideo($('#primary').find('.nayla-video')); ScrollTrigger.refresh(true); naylaMouseCursor(false); setTimeout(function () {
         let siteHeader = $('.site-header'), stickyTargets; ScrollTrigger.refresh()
         if (siteHeader.find('.hide-sticky').length) { stickyTargets = siteHeader.find('.hide-sticky'); }
         siteHeader.removeClass('sticked'); gsap.set([siteHeader, stickyTargets, siteHeader.children('div'), siteHeader.find('.site-logo'), siteHeader.find('.sticky-logo')], { clearProps: 'all' }); naylaHeader(); ScrollTrigger.refresh(true)
      }, 200)
   });
}(jQuery));